...
# Fix the Docker access issue blocking developer router

echo "🔧 FIXING DOCKER ACCESS ISSUE"
echo "============================="
echo ""

cd /home/pi/zoe

# Fix the autonomous_system to handle Docker connection properly
cat > services/zoe-core/autonomous_system_fixed.py << 'FIXED'
...
# Move the fixed version
docker exec zoe-core mv /app/autonomous_system.py /app/autonomous_system_broken.py 2>/dev/null || true
docker exec zoe-core cp /app/autonomous_system_fixed.py /app/autonomous_system.py

# Restart to load fixed version
docker compose restart zoe-core
sleep 10

# Test if it loads now
echo "Testing fixed developer router:"
curl -s http://localhost:8000/api/developer/status | jq '.'

echo -e "\nTesting awareness:"
curl -s http://localhost:8000/api/developer/awareness | jq '.current_state' | head -50

echo -e "\nTesting chat:"
curl -s -X POST http://localhost:8000/api/developer/chat \
  -H "Content-Type: application/json" \
  -d '{"message": "What can you see in the system?"}' | jq -r '.response' | head -200
