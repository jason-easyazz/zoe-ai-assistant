#!/bin/bash
# RECONNECT_ALL_PROVIDERS.sh
# Complete AI client with all providers

echo "🔌 RECONNECTING ALL PROVIDERS"
echo "=============================="
echo ""

cd /home/pi/zoe

cat > services/zoe-core/ai_client_complete.py << 'COMPLETE_AI'
"""Complete AI Client with all providers"""
import os
import sys
import httpx
...
# Replace current ai_client
docker exec zoe-core cp /app/ai_client.py /app/ai_client_old.py
docker exec zoe-core cp /app/ai_client_complete.py /app/ai_client.py

# Restart
docker compose restart zoe-core
sleep 10

echo "✅ All providers connected!"
echo ""
...
echo -e "\nChecking which providers have keys:"
docker exec zoe-core python3 -c "
import os
providers = ['OPENAI', 'ANTHROPIC', 'GOOGLE', 'MISTRAL', 'COHERE', 'GROQ', 'TOGETHER', 'PERPLEXITY']
for p in providers:
    key = os.getenv(f'{p}_API_KEY', '')
    print(f'{p}: {'✓ Configured' if key and key != 'your-key-here' else '✗ No key'}')
"
