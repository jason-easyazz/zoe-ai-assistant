...
def trim_to_speech_end(audio: np.ndarray, chunk: int, floor: int) -> np.ndarray:
    ...
    back to the last chunk carrying real energy so the ground truth is the moment
    the speaker actually stopped.
    """
    last_voiced = -1
    for start in range(0, len(audio) - chunk + 1, chunk):
        if np.abs(audio[start:start + chunk].astype(np.int32)).mean() >= floor:
...
def measure(mod: types.ModuleType, samples: list[np.ndarray], gaps_ms: list[int],
    ...
    for utt in samples:
        ...
        tails.append(1000.0 * (len(utt) if closed is None else max(closed - len(utt), 0)) / rate)
...
def main() -> int:
    ...
    for path in reversed(files):
        ...
        if audio is not None:
            audio = trim_to_speech_end(audio, chunk, mod.RECORD_SILENCE_AMPLITUDE)
