#!/bin/bash
# MAKE_ZACK_GENIUS_AGAIN.sh
# Makes Zack proactively intelligent with full AI integration

set -e

echo "🧠 MAKING ZACK GENIUS LEVEL INTELLIGENT"
echo "========================================"
echo ""
echo "This will make Zack:"
echo "  ✨ Proactively suggest improvements"
echo "  🔍 Analyze the entire system deeply"
echo "  💡 Generate creative solutions"
echo "  🚀 Build features autonomously"
echo ""
echo "Press Enter to upgrade Zack to genius level..."
read

cd /home/pi/zoe

# Step 1: Fix the task system error first
echo "🔧 Step 1: Fixing task system..."
...
conn.commit()
conn.close()
print("✅ Task table ensured")
PYTHON

# Step 2: Create the GENIUS version of developer.py
echo -e "\n🧠 Step 2: Creating genius-level Zack..."
...
# Deploy the genius version
docker cp services/zoe-core/routers/developer_genius.py zoe-core:/app/routers/developer.py

echo "✅ Genius Zack deployed"

# Step 3: Test the genius capabilities
echo -e "\n🧪 Step 3: Testing genius capabilities..."

# Restart to load new code
docker compose restart zoe-core
sleep 10

echo -e "\n📊 Test 1: Deep System Analysis"
curl -s http://localhost:8000/api/developer/analyze | jq '.' | head -20

echo -e "\n💡 Test 2: Proactive Suggestions"
curl -s -X POST http://localhost:8000/api/developer/suggest | jq '.'

echo -e "\n🧠 Test 3: Intelligent Chat"
curl -s -X POST http://localhost:8000/api/developer/chat \
  -H "Content-Type: application/json" \
  -d '{"message": "analyze the system and suggest improvements"}' | jq -r '.response' | head -30

echo -e "\n📋 Test 4: Task Creation (Fixed)"
curl -s -X POST http://localhost:8000/api/developer/tasks \
  -H "Content-Type: application/json" \
  -d '{"title": "Implement caching", "description": "Add Redis caching for API responses", "priority": "high"}' | jq '.'

echo -e "\n✨ GENIUS UPGRADE COMPLETE!"
echo "================================"
echo ""
echo "Zack can now:"
echo "  🧠 Analyze system deeply and proactively"
echo "  💡 Suggest improvements before you ask"
echo "  🚀 Generate implementation plans"
echo "  📊 Track health score (0-100%)"
echo "  🔧 Recommend fixes for issues"
echo "  ✨ Think creatively about solutions"
echo ""
echo "Try asking Zack:"
echo '  "What improvements would make Zoe better?"'
echo '  "Build a notification system"'
echo '  "How can we optimize performance?"'
echo '  "What features are we missing?"'
