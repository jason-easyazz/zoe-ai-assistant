#!/usr/bin/env python3
"""Probe Zoe's Graphiti runtime compatibility without graph writes."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Probe Zoe Graphiti runtime compatibility")
    parser.add_argument("--no-process-scan", action="store_true", help="skip ps/docker process discovery")
    parser.add_argument("--json", action="store_true", help="emit JSON")
    args = parser.parse_args()

    result = probe_graphiti_runtime_sync(include_process_scan=not args.no_process_scan)
    if args.json:
