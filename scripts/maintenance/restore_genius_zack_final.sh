#!/bin/bash
# RESTORE_GENIUS_ZACK_FINAL.sh
# Restores FULL genius capabilities while keeping database compatibility

set -e

echo "🧠 RESTORING GENIUS ZACK WITH FULL INTELLIGENCE"
echo "================================================"
echo ""
echo "Current problem: Zack only says 'System health: 88%'"
echo "Solution: Restore ALL genius features + database compatibility"
echo ""
echo "Press Enter to restore Zack's genius..."
read

cd /home/pi/zoe

# Step 1: Create the COMPLETE genius developer.py with database compatibility
echo "🚀 Step 1: Creating GENIUS developer.py with full intelligence..."
cat > services/zoe-core/routers/developer_genius_final.py << 'PYTHON'
"""
...
docker cp services/zoe-core/routers/developer_genius_final.py zoe-core:/app/routers/developer.py

# Step 2: Restart service
echo -e "\n🔄 Step 2: Restarting with GENIUS capabilities..."
docker compose restart zoe-core
sleep 10

# Step 3: Test genius capabilities
echo -e "\n🧪 Step 3: Testing GENIUS Zack..."

echo "Test 1 - Deep Analysis:"
curl -s -X POST http://localhost:8000/api/developer/chat \
  -H "Content-Type: application/json" \
  -d '{"message": "Analyze the system deeply and suggest innovative improvements"}' | jq -r '.response' | head -50

echo -e "\n" 
echo "Test 2 - Creative Features:"
curl -s -X POST http://localhost:8000/api/developer/chat \
  -H "Content-Type: application/json" \
  -d '{"message": "What cutting-edge features should we add?"}' | jq -r '.response' | head -30

echo -e "\n"
echo "Test 3 - Status Check:"
curl -s http://localhost:8000/api/developer/status | jq '.'

echo -e "\n✨ GENIUS ZACK RESTORED!"
echo "========================"
echo ""
echo "Zack now has:"
echo "  🧠 Deep analytical thinking"
echo "  💡 Creative feature suggestions"
echo "  🚀 Proactive improvements"
echo "  📊 Real system insights"
echo "  🎯 Implementation planning"
echo "  ✨ Genius-level responses"
echo ""
echo "Try asking:"
echo '  "What would make Zoe absolutely revolutionary?"'
echo '  "Design a feature that would blow peoples minds"'
echo '  "How can we make Zoe 100x better?"'
