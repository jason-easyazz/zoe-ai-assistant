#!/bin/bash
# RESTORE_ZACK_EXECUTION.sh
# Restores ACTUAL command execution to Zack instead of hallucinations

set -e

echo "🔧 RESTORING ZACK'S ACTUAL COMMAND EXECUTION"
echo "==========================================="
echo ""
echo "Current problem: Zack just hallucinates responses"
echo "Solution: Make him ACTUALLY run commands and return real data"
echo ""
echo "Press Enter to fix this once and for all..."
read

cd /home/pi/zoe

# Backup current
docker exec zoe-core cp /app/routers/developer.py /app/routers/developer_hallucinating.py

# Create the WORKING version that ACTUALLY EXECUTES
cat > services/zoe-core/routers/developer_fixed.py << 'EOF'
"""
...
docker cp services/zoe-core/routers/developer_fixed.py zoe-core:/app/routers/developer.py
...
echo "Test 1: List actual files"
curl -s -X POST http://localhost:8000/api/developer/chat \
  -H "Content-Type: application/json" \
  -d '{"message": "List router files"}' | jq -r '.response' | head -20

echo -e "\nTest 2: Check real system health"
curl -s -X POST http://localhost:8000/api/developer/chat \
  -H "Content-Type: application/json" \
  -d '{"message": "Check system health"}' | jq -r '.executed'

echo -e "\nTest 3: Get actual container status"
curl -s -X POST http://localhost:8000/api/developer/chat \
  -H "Content-Type: application/json" \
  -d '{"message": "Show docker containers"}' | jq -r '.commands_run'

echo -e "\n================================================"
echo "✅ ZACK NOW EXECUTES REAL COMMANDS!"
echo "================================================"
echo ""
echo "What's fixed:"
echo "  ✅ Real subprocess.run() execution"
echo "  ✅ Actual docker ps output"
echo "  ✅ Real database queries"
echo "  ✅ Actual file listings"
echo "  ✅ No more hallucinations"
echo ""
echo "Test in the UI:"
echo "  http://192.168.1.60:8080/developer/chat.html"
echo ""
echo "Ask Zack:"
echo '  "Show docker containers" - Returns REAL docker ps'
echo '  "List database tables" - Returns REAL tables'
echo '  "Check system health" - Runs REAL health checks'
echo '  "Show router files" - Lists REAL files'
echo ""
echo "The chat will now show ACTUAL system data, not fantasies!"
