...
- **Now (🔨):** spike feasibility half ran (2026-07-05) → **decision made 2026-07-06:** [`docs/architecture/ambient-voice-migration-plan.md`](architecture/ambient-voice-migration-plan.md) — **NO-GO on a full Pipecat migration now** (Moonshine not native to Pipecat; numpy/torch ABI risk; memory-saturated box forces replace-not-parallel), **GO on the ADR's fallback**: borrow **Smart Turn v3** + hand-build **barge-in** on the existing LiveKit agent — phased A0–A3, flag-gated, replay-gated, live path untouched until the live-mic acceptance session passes. Pipecat parked with explicit re-open triggers. **NEXT: Phase A0** (baseline + Smart Turn standalone bench). **Phone (SIP) + Wyoming room satellites remain later, separable increments.**
