...
- **Now:** Phase 0 (harness on Flue) ✅ #858 (+1.5 ms voice). Phase 1 (Telegram as a front-door channel) 🔨 — bot built (#870), being re-slotted through `/api/chat` with a `channel` tag. Flue brain recall/tool reliability work in flight (#915 series). **NEXT:** Phase 2 (Flue brain behind the `run_zoe_core` seam, lab-proved on the voice corpus) + start the §8.1 Multica recreation (durable ticket workflow), gates before any deletion.
