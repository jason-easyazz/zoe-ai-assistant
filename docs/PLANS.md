...
- **Now:** 📝 planned, not started. **Every claim verified against the live tree/host 2026-07-04 (3 read-only sub-agents); the plan doc has the corrections.** **Wave 1** (docs/hygiene, zero runtime risk) — PR A: reconcile the brain docs to reality (live brain = the `flue` sidecar, not `services/zoe-core` which is the dormant fallback; `.cursorrules`/READMEs are stale, `zoe-flue-integration.md` is the accurate anchor); PR B drift sweep (HARDWARE_COMPAT/graphify/agent-zero-research — all confirmed removable, don't delete HARDWARE_COMPAT file); PR C untrack runtime artifacts. **Wave 2** CI gate — marker-based `ci_safe` selection (⚠️ NOT blanket `testpaths`: ~25 e2e/integration tests fail-not-skip without the host) + post-deploy self-hosted pytest. **Wave 3 (root cause CORRECTED):** the `ok:false` bugs are a broken `mcporter` subprocess fallback, NOT triplication → Tier 1 fix the fallback (live gap: `people_relate` fails today) THEN Tier 2 dedup into services (replay-gated). **Wave 4** god-files/typed config/fence harness. **3 live issues surfaced:** `people_relate` broken now; `docs/archive/` 241 untracked files (don't `git add`); orphaned `zoe-graphify-refresh.service`.
