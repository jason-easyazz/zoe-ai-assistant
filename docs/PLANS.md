...
- **Now:** Phase 0 (harness on Flue) ✅ #858 (+1.5 ms voice). **Brain lane (phases 1–2 of the brain campaign = detail-doc Phases 2–3) ✅ delivered:** #947 (event-loop fork-deadlock fix — 3.9-day `/api/memories/for-prompt` outage; now 18–66 ms), #915/#939 (recall reliability 97% + turn cap), #944 (non-mutating dry-run preflight), #960 (`reminder_list` direct executor), #904 (`ZOE_BRAIN_BACKEND=flue` seam, default OFF), #952 (progressive tool disclosure 11→3 schemas + systemd unit template + runbook), #965 (activator fallback hardening), #971 (Seam-A sentinel streaming). E2E-verified through the seam; ~2× faster than the prod core on pure chat. **Voice-parity gate RUN 2026-07-03: latency clear win, quality ~parity — median 2.5s vs 5.3s (clean, unconfounded); raw quality flue 41/44 vs prod 38/44 OVERSTATES flue (identity confound: prod ran as guest w/ contaminated memory, flue as empty-store test user). Removing the 4 identity-decided prompts → flue 37/40 (92.5%) vs prod 38/40 (95%), prod marginally ahead on quality. Clean quality re-run needs an authenticated test user (blocked by zoe-auth provisioning).** On-box LANDING checks PASS (sentinel stream; activator 7/10 trigger-free, zero fabrication; recall 96.9%). **Still a lab sidecar, NOT canonical** — voice-degrading blockers since FIXED: "I don'm" token corruption (#991, temperature parity 0.5) + in-session context recall (#988); remaining: tool coverage 12 → 20 via Waves 1–3 (remainder deliberately cut per [`docs/knowledge/flue-cutover-tool-cut-list.md`](knowledge/flue-cutover-tool-cut-list.md), signed off 2026-07-03); write path still unexercised live. Phase 1 (Telegram front door) 🔨 — bot built (#870), re-slot through `/api/chat` with a `channel` tag still open. **NEXT:** the flip is now the operator's call, but fix the two NEW voice-degrading blockers first; §8.1 Multica recreation after that, gates before any deletion.
