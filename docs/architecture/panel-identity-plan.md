...
- **Shadow week (mandatory before acting — W5 gate):** the daemon ships `SPEAKER_ID_SHADOW=true` (default). With it on, every turn is scored and LOGGED — journal `Speaker ID (shadow)` lines plus JSONL rows `{boot, seq, ts, panel_id, user_id, score, n_profiles, source, truth}` (user_id null on no-match; never audio or embeddings, per `docs/knowledge/biometric-retention-policy.md`) appended to `~/.zoe-voice/speaker_shadow_metrics.jsonl` (override `SPEAKER_ID_SHADOW_LOG`) — but the claim is NEVER attached to the turn payload, so the server cannot act on identity.

  **The artifact holds predictions, not outcomes — FA/FR is not computable from it unlabelled.** The daemon cannot know who actually spoke, so `truth` ships null and the operator fills it during review: the **`(boot, seq)` pair** is the stable per-row handle — `boot` is a fresh token per daemon start and `seq` counts within that start, so a restart mid-week cannot reuse a handle and uniqueness never depends on successfully re-reading the existing log; `ts` locates the turn, and `n_profiles` records how many enrolled voices the score was chosen from — **null when `source` is `server`**, since a claim resolved by the `/api/voice/identify` fallback was scored against a profile set the panel never saw. A `source` of **`error`** means the identify attempt itself failed (network/server) — exclude those rows too, and a run of them means the panel↔server path is broken, not that nobody matched. A `source` of **`encoder_unavailable`** means the panel could not load resemblyzer at all — those rows were never scored, so exclude them from FA/FR entirely: a week of them is an installation failure, not data (the installer WARNS loudly but continues — speaker ID ships disabled, so a panel that never enables it must still provision; the journal flags each such turn as `NOT SCORED` at WARNING). A failed metrics write is logged as `NOT logged` in the journal rather than counted, so the journal and the artifact cannot disagree. Labelling protocol for ≥1 week of rows: while only Jason is enrolled, mark `truth` as his user_id for turns he spoke and `"other"` for anyone else (rows nobody can place stay null and are excluded). One asymmetry the formulas must respect: **local rows are raw candidates, server rows are threshold-resolved.** `_match_speaker_local` always reports the best candidate with its raw cosine score (the acceptance decision belongs to the server), so a `source='local'` row's `user_id` does NOT mean "accepted" — first apply the operative `ZOE_SPEAKER_ID_THRESHOLD` to its `score` and treat sub-threshold rows as no-match. `source='server'` rows are already resolved. Then **false accept** = `truth != user_id` where user_id is non-null (after thresholding local rows), **false reject** = `truth` is an enrolled user where user_id is null or a different user. Review those two numbers with the operator, tune `ZOE_SPEAKER_ID_THRESHOLD` (server-side — the daemon always sends its best claim and the server owns acceptance), and only then set `SPEAKER_ID_SHADOW=false` + restart. Rollback: `SPEAKER_ID_ENABLED=false` + restart; delete enrollment via the retention policy's self-service deletion path. **Retention of the shadow artifact itself:** `speaker_shadow_metrics.jsonl` is a review-window artifact, not a permanent record — it holds per-turn predicted identities, operator-filled ground truth, confidences and timestamps. Delete it (`(set -a; . ~/.zoe-voice/.env.voice 2>/dev/null; rm -f "${SPEAKER_ID_SHADOW_LOG:-$HOME/.zoe-voice/speaker_shadow_metrics.jsonl}")` — sources the panel env first, so a custom `SPEAKER_ID_SHADOW_LOG` set only in `.env.voice` is honoured rather than silently deleting just the default) when the shadow-week review concludes, when shadow mode is turned off, and as part of any enrollment deletion; a panel should never carry it indefinitely.
