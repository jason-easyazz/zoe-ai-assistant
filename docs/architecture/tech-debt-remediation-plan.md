...
- **Test-isolation leak (exposed by the CI marker mechanism)** — 3 tests pass in isolation but fail in the full `-m ci_safe` run (`test_relationship_graph::test_endpoint_flag_on_returns_graph` / `::test_endpoint_unknown_person_404` → 403 instead of 200/404; the whole `test_zoe_core_client` file). Root cause is a cumulative cross-test leak of `guest_policy`/auth global state that forces a guest verdict (not any single file; the shifting victim set depends on run composition). Excluded on the fast GitHub lane (they still run on the self-hosted full-dir lane); fix = find + isolate the leaking test's global mutation, then drop the `--deselect`s in `validate.yml`.
