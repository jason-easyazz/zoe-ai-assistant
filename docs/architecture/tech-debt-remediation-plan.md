...
- **Test-isolation leak (exposed by the CI marker mechanism)** — 3 tests pass in isolation but fail in the full `-m ci_safe` run (`test_relationship_graph::test_endpoint_flag_on_returns_graph` / `::test_endpoint_unknown_person_404` → 403 instead of 200/404; `test_zoe_core_client::test_web_query_delegates_and_synthesizes`). Root cause is a cumulative cross-test leak of `guest_policy`/auth global state that forces a guest verdict (not any single file; the first ~7 ci_safe files together trip it). Deselected on the fast GitHub lane (they still run on the self-hosted full-dir lane); fix = find + isolate the leaking test's global mutation, then drop the `--deselect`s in `validate.yml`.
...
- **Loopback identity-override (security)** — `_is_internal_request` (`services/zoe-data/auth.py`) trusts bare loopback with NO token, and that trust now includes acting-as-any-user on `/api/chat` (`resolve_acting_user` honors `X-Zoe-User-Id`). Nginx now strips the header at the edge (#1020), so the *external* vector is closed; the residual is a loopback SSRF/compromised-skill inheriting impersonation. **Proper fix needs `ZOE_INTERNAL_TOKEN` provisioned first** (currently unset live — verified), then require a valid token (not loopback alone) for the override; requiring it now would break Telegram identity (the bridge sends the token only if configured). Tracked so it doesn't evaporate.
