...
→ **Lab-prove the composed packet (2a corpus run + 2b/2c enable in a worktree vs real Chroma), then
increment 3a** — in a worktree (never prod), enable `ZOE_MEMORY_COMPOSE_ENABLED` (and
`ZOE_HYBRID_RETRIEVAL_ENABLED`) against real Postgres/Chroma and confirm on **both** the chat
`/api/memories/for-prompt` packet **and** the voice `_voice_recall_packet` block that a
person/relationship/date query returns the cited relational facts alongside vector recall (and a
non-relational query stays vector-only), then re-run the 2a recall set + saved-audio corpus
(`~/.zoe-voice-samples` via `scripts/perf/measure_voice.py` under the flock) confirming no regression
and no cross-user/unapproved leakage (hot path stays warm sub-400 ms). With the composed packet proven,
start **increment 3a** — the idle *reflection pass*: synthesize higher-level insights from recent
memories into `user_portraits` + importance-scored memories. Update §6/§7 after the step.
