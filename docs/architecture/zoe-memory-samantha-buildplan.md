...
→ **Lab-prove the composed packet (2b) + run the realistic recall-set/saved-audio corpus for 2a on
the box** — in a worktree (never prod), enable `ZOE_MEMORY_COMPOSE_ENABLED` (and
`ZOE_HYBRID_RETRIEVAL_ENABLED`) against real Postgres/Chroma, confirm a person/relationship/date
query returns the cited relational block alongside vector recall (and a non-relational query stays
vector-only), and re-run the 2a recall set + saved-audio corpus (`~/.zoe-voice-samples` via
`scripts/perf/measure_voice.py` under the flock) confirming no regression and no cross-user/unapproved
leakage (hot path stays warm sub-400 ms) — then **2c** (point voice `_voice_recall_packet` at the same
composed packet). Update §6/§7 after the step.
