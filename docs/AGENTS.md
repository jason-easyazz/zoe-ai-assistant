...
- [knowledge/operator-skills/](knowledge/operator-skills/index.md) — retirement archive for the Hermes + OpenClaw operator skills: every skill under `~/.hermes/skills` and `~/.openclaw` that is not recoverable from a public source, plus a named source for each exclusion. Preservation only; read by no runtime, and NOT a substitute for `skills/` (whose contract forbids operator skills). Restore is manual. Archive coverage is the precondition for deleting the live installs — verify it before any `rm`.
