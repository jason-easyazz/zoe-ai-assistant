...
class MetadataRecommendationEngine(BaseRecommendationEngine):
        ...
        try:
            ...
            if client:
                ...
                if watch and "tracks" in watch:
                    ...
                    logger.info(f"Got {len(related)} similar tracks from watch playlist")
        except Exception as e:
            logger.warning(f"Watch playlist failed: {e}")
        
        # Fallback: search for similar based on track metadata
        if not related:
            # Try cache first
            track_info = await self._get_track_info(track_id)
            
            # If not in cache, try to fetch from YouTube Music
            if not track_info or not (track_info.get('title') or track_info.get('artist')):
                try:
                    fetched = await self.youtube.get_track(track_id, user_id)
                    if fetched:
                        track_info = {
                            'title': fetched.get('title', ''),
                            'artist': fetched.get('artist') or (fetched.get('artists', [{}])[0].get('name', '') if fetched.get('artists') else ''),
                            'album': fetched.get('album', '')
                        }
                        logger.info(f"Fetched track info for {track_id}: {track_info}")
                except Exception as e:
                    logger.warning(f"Failed to fetch track info: {e}")
            ...
            if track_info:
                ...
                if artist:
                    # Search for more songs by the same artist first
                    logger.info(f"Searching for similar to: {artist} - {title}")
                    results = await self.youtube.search(f"{artist} songs", user_id, limit=limit)
                    related.extend(results)
                    
                    # Then search for similar artists
                    if len(related) < limit:
                        results = await self.youtube.search(f"artists like {artist}", user_id, limit=5)
                        related.extend(results)
                elif title:
                    # Search by title if no artist
                    results = await self.youtube.search(f"songs like {title}", user_id, limit=limit)
                    related.extend(results)
        
        # Ultimate fallback: search for trending music
        if not related:
            logger.info(f"No similar tracks found for {track_id}, using trending fallback")
            related = await self.youtube.search("top hits 2024", user_id, limit=limit * 2)
        
        # Dedupe (excluding seed track) and rank by affinity
        exclude_ids = {track_id}  # Exclude the seed track
        unique = self._dedupe_tracks(related, exclude_ids)
    ...
    async def get_radio(
        ...
        """
        # Track IDs to exclude (seed track if provided)
        exclude_ids = {seed_track_id} if seed_track_id else set()
        ...
        if not top_tracks:
            ...
            results = await self.youtube.search("popular music 2024", user_id, limit=limit * 2)
            return self._dedupe_tracks(results)[:limit]
        
        # Add top tracks to exclusions
        ...
        unique = self._dedupe_tracks(all_similar, exclude_ids)
        ranked = self.affinity.rank_tracks_by_affinity(unique, user_id)
    ...
    async def get_discover(
        ...
        Uses YouTube Music charts for trending data when available.
        """
        # Get user's top artists
        top_artists = self.affinity.get_top_artists(user_id, limit=5)
        
        # Get already played tracks to exclude
        played = await self._get_played_tracks(user_id)
        
        discoveries = []
        
        # Try to get real charts/trending data first
        chart_tracks = await self._get_chart_tracks(user_id, limit=10)
        if chart_tracks:
            discoveries.extend(chart_tracks)
            logger.info(f"Got {len(chart_tracks)} tracks from charts")
        
        if not top_artists:
            # New user - use charts or fallback to search
            if not discoveries:
                results = await self.youtube.search("new music 2024", user_id, limit=limit * 2)
                discoveries.extend(results)
            unique = self._dedupe_tracks(discoveries, played)
            return unique[:limit]
        ...
        # Dedupe, excluding played tracks
        unique = self._dedupe_tracks(discoveries, played)
    ...
    async def _get_chart_tracks(self, user_id: str, limit: int = 10) -> List[Dict]:
        """
        Get trending tracks from YouTube Music charts.
        
        Uses get_charts() API which returns real trending/chart data.
        """
        try:
            client = await self.youtube.get_client(user_id)
            if not client:
            ...
            # get_charts returns: songs, videos, artists, trending (videos)
            # We want songs from the charts
            charts = client.get_charts(country="US")  # Could make country configurable
            
            tracks = []
            
            # Get from "Top Songs" section
            if charts and "songs" in charts:
                songs = charts["songs"]
                items = songs.get("items", [])
                for item in items[:limit]:
                    if item.get("videoId"):
                        tracks.append(self.youtube._normalize_track(item))
            
            # Also try trending videos (often music videos)
            if len(tracks) < limit and charts and "trending" in charts:
                trending = charts["trending"]
                items = trending.get("items", [])
                for item in items[:limit - len(tracks)]:
                    if item.get("videoId"):
                        tracks.append(self.youtube._normalize_track(item))
            ...
            logger.warning(f"Failed to get charts: {e}")
            return []
    ...
    def _dedupe_tracks(self, tracks: List[Dict], exclude_ids: Set[str] = None) -> List[Dict]:
        """Remove duplicate tracks by videoId and title+artist combination.
        
        Args:
            tracks: List of track dicts
            exclude_ids: Optional set of track IDs to exclude (e.g., currently playing)
            
        Returns:
            Deduplicated list of tracks
        """
        seen_ids = set(exclude_ids or set())
        seen_titles = set()  # Also track by title+artist to catch same song different IDs
        unique = []
        ...
        for track in tracks:
            ...
            # Also check by title+artist to catch duplicates with different IDs
