...
- **Status (2026-07-29):** the graph-adjacency boost to `_blend` is **built and merged** (PR #1199, 2026-07-09 — pure Postgres recursive-CTE, **no new models, ~zero RAM**). Per the enable runbook ([`relationship-memory-flag-enable.md`](knowledge/relationship-memory-flag-enable.md), Enable log), **both `ZOE_RELATIONSHIP_GRAPH_ENABLED=1` and `ZOE_GRAPH_RECALL_BOOST=1` have been live in prod since 2026-07-22** (verified in live `.env`; migration 0015 applied — both flags error at runtime without it). The feature is therefore **active, not dark** — the flags ship OFF by default but are enabled in prod. The old “RAM-blocked / lands after W0” framing never applied: go-live (07-22) predated PR #1572's replay-gate RAM work (07-27) by five days, so the boost's enablement was never gated on it.
- **Next:** not an enable (already live) — **run the multi-hop eval shipped with #1199 and measure the actual recall lift against GBrain's +31 P@5 benchmark**, and record the number here. Any rollback/re-enable must name **both** flags (`ZOE_RELATIONSHIP_GRAPH_ENABLED` + `ZOE_GRAPH_RECALL_BOOST`) — flag-off only, never schema-downgrade. **Reject from GBrain:** hosted embeddings (breaks the local/private rock) and a resident local reranker like Qwen3-Reranker (breaks the RAM budget — see the 2026-07-08 25 GB-swap incident).
