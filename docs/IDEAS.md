...
- **Status (2026-07-29):** the graph-adjacency boost to `_blend` is **already built and merged** (PR #1199, 2026-07-09 — pure Postgres recursive-CTE, **no new models, ~zero RAM**), dark behind `ZOE_RELATIONSHIP_GRAPH_ENABLED` (default OFF). The former RAM blocker is **dissolved**: PR #1572 (2026-07-27) moved the replay gate to remote-STT mode (445 MB measured vs the old 1500 MB) and the gate passes again (19/19) — the “lands AFTER W0” sequencing no longer applies.
- **Next:** operator enable — flip `ZOE_RELATIONSHIP_GRAPH_ENABLED` per its runbook, run the multi-hop eval shipped with #1199, and measure recall lift against GBrain's +31 P@5 benchmark. **Reject from GBrain:** hosted embeddings (breaks the local/private rock) and a resident local reranker like Qwen3-Reranker (breaks the RAM budget — see the 2026-07-08 25 GB-swap incident).
