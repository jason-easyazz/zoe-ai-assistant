...
### `ZOE_VAD_TAIL_MS=640` — the ear-tuned endpointing tail — ✅ DONE (already live)
- **Core:** pinned during the 2026-07-28 review as a pending operator env-flip, but verification before merge found it **already live**: `ZOE_VAD_TAIL_MS=640` is set in the RUNNING Pi voice daemon (pid 423560, read from `/proc/<pid>/environ` on zoe-pi). It is invisible to `systemctl --user show zoe-voice` (the unit's `Environment=` is empty) and absent from zoe-data's env entirely, which is why three documents disagreed about it — `docs/PLANS.md` said "dark" (correction pending in #1581 — if that has merged, PLANS reads LIVE; if not, PLANS is still the stale one), the #1581 rollout record said "staged live", and this pin said not-yet-flipped. Kept as a DONE record rather than deleted, so the next reader inherits the check-the-right-place lesson. Remaining follow-up: re-run the endpointing probe as the before/after instrument. Source: 2026-07-28 review (backlog record: ZOE-6134).
