...
- **Core:** face-ID has enrollment/retention machinery but no user-facing enroll/review/delete flow shipped yet. **Correction on review:** the flow IS already specified — `docs/architecture/panel-identity-plan.md` § "Enrollment UX (Phase 2/3)" defines it (Settings "Face Identity" card beside Voice Identity → consent text → 3-pose capture → `POST /api/face/enroll` → list/delete profiles, explicit opt-in per person per W6 consent), and `docs/PLANS.md` tracks enrollment UX as the next step of that active workstream. So this is **not** unscoped and does not await a sketch: what it needs is BUILD time against the existing plan — and less than it sounds: the guided three-pose capture already exists operator-side (`scripts/setup/zoe_enroll_flow.py` guides the person, records, embeds, POSTs `/api/face/enroll`, refreshes the panel cache), so the true gap is exposing that flow through the Settings card — plus Jason's call on the one genuinely open detail (consent wording — per-person delete already lives in the same Face Identity card per the plan's list/delete spec). Retention *policy* is settled (`docs/knowledge/biometric-retention-policy.md`: kept until owner/user deletion, no time-based expiry); ZOE-6129 covers the remaining deletion UI/cache gaps it documents, not an open policy question. Source: 2026-07-28 review (backlog record: ZOE-6134); corrected 2026-07-28 against the panel-identity plan.
