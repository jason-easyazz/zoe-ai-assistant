...
2. Fresh Graphify and architecture inventory.
   ...
   - Use the scoped local model matrix evidence to prioritize E4B for the next full-repo probe, then build a reliable local/offline Graphify refresh lane with bounded chunking, JSON repair/validation, and partial-output discard before treating Graphify as current for major cleanup.
