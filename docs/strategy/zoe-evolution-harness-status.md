...
- Graphify has been refreshed through current main source commit `dbe4474b`
  after the Hindsight offline rollout docs merge; the map is current for
...
| Graphify current map | Complete foundation | `graphify-out/GRAPH_REPORT.md` and `graphify-out/graph.json` were regenerated through current main source commit `dbe4474b` after the Hindsight offline rollout docs merge; `docs/architecture/zoe-harness-current-inventory.md` records the source-backed inventory. | Rerun Graphify after substantial code or architecture changes. |
