...
- Graphify was last successfully refreshed through source commit `75f5345d`.
  A refresh attempt after `f644bee` scanned 592 code files and 257 docs but
...
| Graphify current map | Stale after quota-blocked refresh | `graphify-out/GRAPH_REPORT.md` and `graphify-out/graph.json` were last successfully regenerated through source commit `75f5345d`. A post-`f644bee` refresh attempt scanned 592 code files and 257 docs but all 7 OpenAI semantic chunks failed with `insufficient_quota`; the tool still wrote partial output, so it was discarded. | Restore OpenAI quota or approved local/offline Graphify backend, then rerun Graphify before major cleanup. |
...
2. Fresh Graphify and architecture inventory.
   ...
   - Latest blocked attempt after `f644bee` scanned 592 code files and 257 docs; all 7 semantic chunks failed, and the partial graph output was discarded.
