...
- The deterministic memory router now has a disabled-by-default runtime status gate and optional non-persistent observation trace packets; it is not yet used for prompt injection or backend recall in production chat.
...
| Observation/evaluation traces | Complete foundation | `services/zoe-data/zoe_observation_trace.py`, `services/zoe-data/tests/test_zoe_observation_trace.py`, and `docs/architecture/zoe-observation-trace-schema.md` define trace records for memory route decisions, recall, retain candidate, admission, contradiction, fallback, proposal, verification, outcome eval, and hardware-budget events. | Wire governed trace collection around retain candidates, sidecar bake-offs, Multica admission, and capability promotion/retirement. |
...
7. Runtime memory router feature flag.
   ...
   - Next: add a governed trace collector, then wire prompt-time recall in fallback-safe mode with latency timeout and compact cited memory packets.
