...
2. Fresh Graphify and architecture inventory.
   ...
   - Use the no-write shard sync plan (`ready_for_artifact_merge_design`, `artifact_sync_ready=false`) to add artifact-preserving shard runs and deterministic merge validation before treating Graphify as current for major cleanup.
