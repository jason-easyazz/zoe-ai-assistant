...
| Capability profiles | Complete foundation | `services/zoe-data/zoe_capability_profile.py`, `services/zoe-data/zoe_capability_trust_update.py`, tests, and `docs/architecture/zoe-capability-profiles.md` define the first executable Zoe capability self-model and retained-outcome trust-update candidates. | Add a governed profile writer/reviewer that can accept or reject trust-update candidates. |
...
| Self-evolution loop | Partial | Multica, pipeline evidence, Greploop, Greptile, Hermes, worktree bootstrap, candidate scoring, proposal contract, contract-aware Multica admission, live execution approval gating for execute/promote proposal tickets, inert outcome-memory candidate construction, outcome admission decisions, admitted Hindsight outcome retain, and retained-outcome capability trust candidates exist. | Add governed review/promotion for capability trust and continue wiring Notice/Search/Evaluate runtime callers. |
