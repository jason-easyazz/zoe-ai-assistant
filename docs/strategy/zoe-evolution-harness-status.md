...
| Graphiti bake-off | Partial with inert proposal intake | ADR plus `services/zoe-data/graphiti_bakeoff.py`, `services/zoe-data/graphiti_sidecar_probe.py`, `services/zoe-data/graphiti_runtime_probe.py`, `scripts/maintenance/graphiti_runtime_proposal.py`, `scripts/maintenance/graphiti_local_model_probe.py`, `services/zoe-data/zoe_evolution_runtime_intake.py`, tests, and `docs/architecture/zoe-graphiti-fixtures.md` define the first relationship fixture set, backend/runtime/local-model preflights, and a review-only optional-runtime proposal. FalkorDB was reachable in a temporary local sidecar, Zoe's normal Python runtime lacks Graphiti backend packages, Graphiti's library extractor failed an earlier local structured-output smoke test, and the explicit local Gemma contract probe now passes at about 6.1 s. Generated proposals remain blocked until optional dependency, sidecar, structured-output, latency, memory-footprint, and approval evidence exist. | Submit or review the optional-runtime proposal only after operator approval, then run the explicit local model contract probe and solve any structured-output failures before accepted fixture ingest/query measurements. |
