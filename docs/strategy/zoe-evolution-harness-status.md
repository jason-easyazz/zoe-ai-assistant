...
| Memory router | Partial | `services/zoe-data/zoe_memory_router.py`, `services/zoe-data/zoe_memory_router_runtime.py`, `services/zoe-data/tests/test_zoe_memory_router.py`, `services/zoe-data/tests/test_zoe_memory_router_runtime.py`, and `/api/system/memory-router/status` expose disabled-by-default observe-only route decisions with optional non-persistent trace packets. | Route runtime trace packets through the governed collector, then wire prompt-time recall behind a second explicit feature flag with latency guards. |
...
7. Runtime memory router feature flag.
   ...
   - Next: route optional trace packets through the governed collector, then wire prompt-time recall in fallback-safe mode with latency timeout and compact cited memory packets.
