...
| Graphify current map | Stale after quota-blocked refresh | `graphify-out/GRAPH_REPORT.md` and `graphify-out/graph.json` were last successfully regenerated through source commit `75f5345d`. A post-`18b8bd5` refresh attempt scanned 599 code files and 257 docs but all 7 OpenAI semantic chunks failed with `insufficient_quota`; the tool still wrote partial output, so it was discarded. Local Ollama/Gemma completed a semantic smoke test, but a full-repo attempt hit repeated context splits and invalid JSON chunks before it was stopped. | Restore OpenAI quota or validate a more reliable approved local/offline Graphify backend, then rerun Graphify before major cleanup. |
