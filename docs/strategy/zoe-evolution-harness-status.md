...
- Graphify has been refreshed through current main source commit `d58ac5e3` after the capability-profile promotion manifest merge; the map is current for source surfaces until the next code or architecture change.
...
- Zoe now has a deterministic capability-profile promotion manifest gate that
  requires clean trust reviews plus PR, rollback, and verification refs before
  any future profile writer can render applyable promotion records.
...
| Graphify current map | Complete foundation | `graphify-out/GRAPH_REPORT.md` and `graphify-out/graph.json` were regenerated through current main source commit `d58ac5e3` after the capability-profile promotion manifest merge; `docs/architecture/zoe-harness-current-inventory.md` records the source-backed inventory. | Rerun Graphify after substantial code or architecture changes. |
...
| Capability profiles | Complete foundation | `services/zoe-data/zoe_capability_profile.py`, `services/zoe-data/zoe_capability_profile_promotion.py`, `services/zoe-data/zoe_capability_trust_update.py`, `services/zoe-data/zoe_capability_trust_review.py`, tests, and `docs/architecture/zoe-capability-profiles.md` define the first executable Zoe capability self-model, retained-outcome trust-update candidates, governed in-memory trust reviews, and promotion manifests with PR/rollback/verification evidence. | Add an explicit profile-file patch writer that consumes promotion manifests and still lands through PR review. |
...
| Self-evolution loop | Partial | Multica, pipeline evidence, Greploop, Greptile, Hermes, worktree bootstrap, candidate scoring, proposal contract, contract-aware Multica admission, live execution approval gating for execute/promote proposal tickets, inert outcome-memory candidate construction, outcome admission decisions, admitted Hindsight outcome retain, retained-outcome capability trust candidates, governed in-memory trust reviews, and promotion manifests exist. | Add profile-file patch writing plus Notice/Search/Evaluate runtime callers. |
