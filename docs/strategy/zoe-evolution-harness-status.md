...
| Pi runtime harness | Partial with inert proposal intake | `services/zoe-data/pi_runtime_probe.py`, `scripts/maintenance/pi_runtime_probe.py`, `scripts/maintenance/pi_runtime_proposal.py`, `services/zoe-data/zoe_evolution_runtime_intake.py`, tests, and `docs/architecture/zoe-pi-runtime-harness.md` detect Pi readiness and can build a review-only runtime-adoption proposal without installing or executing Pi. Current Zoe host lacks Node/npm/Pi, and generated proposals remain blocked by offline/runtime prerequisites. | Submit or review the proposal only after operator approval for runtime installation and local model configuration evidence; keep delegated Pi execution disabled until the probe reports available runtime and Multica approves use. |
...
12. Pi runtime harness.
    - Status: read-only probe, policy contract, and inert install/runtime proposal builder complete.
    - Current host evidence still reports Node/npm/Pi missing and execution disabled.
    - Next: operator-approved runtime installation and local/offline model config evidence before delegated Pi execution.
