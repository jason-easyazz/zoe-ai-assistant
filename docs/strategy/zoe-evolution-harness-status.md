...
- Graphify has been refreshed from Hindsight retain-executor merge source commit `5669f90` after PR #314; the map is current for source surfaces until the next code or architecture change.
...
| Graphify current map | Complete foundation | `graphify-out/GRAPH_REPORT.md` and `graphify-out/graph.json` were regenerated from Hindsight retain-executor merge source commit `5669f90` after PR #314; `docs/architecture/zoe-harness-current-inventory.md` records the source-backed inventory. | Rerun Graphify after substantial code or architecture changes. |
