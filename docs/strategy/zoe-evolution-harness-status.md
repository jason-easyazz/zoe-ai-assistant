...
- Graphify has been refreshed through current main source commit `beeb4374` after the capability-trust candidate and nginx header helper merges; the map is current for source surfaces until the next code or architecture change.
...
| Graphify current map | Complete foundation | `graphify-out/GRAPH_REPORT.md` and `graphify-out/graph.json` were regenerated through current main source commit `beeb4374` after the capability-trust candidate and nginx header helper merges; `docs/architecture/zoe-harness-current-inventory.md` records the source-backed inventory. | Rerun Graphify after substantial code or architecture changes. |
...
| Capability profiles | Complete foundation | `services/zoe-data/zoe_capability_profile.py`, `services/zoe-data/zoe_capability_trust_update.py`, `services/zoe-data/zoe_capability_trust_review.py`, tests, and `docs/architecture/zoe-capability-profiles.md` define the first executable Zoe capability self-model, retained-outcome trust-update candidates, and governed in-memory trust reviews. | Add an explicit profile writer that can persist reviewed promotions through PR evidence and rollback. |
...
| Self-evolution loop | Partial | Multica, pipeline evidence, Greploop, Greptile, Hermes, worktree bootstrap, candidate scoring, proposal contract, contract-aware Multica admission, live execution approval gating for execute/promote proposal tickets, inert outcome-memory candidate construction, outcome admission decisions, admitted Hindsight outcome retain, retained-outcome capability trust candidates, and governed in-memory trust reviews exist. | Add persisted profile promotion plus Notice/Search/Evaluate runtime callers. |
