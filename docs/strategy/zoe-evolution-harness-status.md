...
| Offline-only memory rule | Partial | `HindsightConfig` rejects public/cloud memory LLM and embedding configuration by default, and the Hindsight embedding probe can verify local model/service readiness without downloads or writes. | Run the embedding preflight on Zoe before any live Hindsight bake-off and install/configure only local/offline embedding dependencies. |
...
| Hindsight bake-off | Partial | Offline sidecar client, retain-candidate helpers, synthetic fixtures, measured runner, read-only sidecar probe, read-only embedding preflight, availability doc, admission bridge, and tests exist; current Zoe-host check found no running Hindsight sidecar and no cached default embedding model. | Start a local/offline sidecar only after the embedding probe reports a local model/service available, run retain/recall with synthetic events, and record p50/p95 latency, failures, evidence quality, and CPU/RAM use. |
