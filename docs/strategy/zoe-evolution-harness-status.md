...
- Graphify was last successfully refreshed through source commit `75f5345d`.
  A refresh attempt after `b69b638` scanned 595 code files and 257 docs but
...
- Zoe now has an inert Multica handoff packet builder and runtime-callable
  orchestration plan for governed capability profile promotions: reviewed trust
  results can be converted into promotion manifests, patch plans, and handoff
  packets while round-tripping ticket metadata, carrying manifest and patch
  hashes, and refusing blocked payloads before any future ticket writer can
  consume them.
...
| Graphify current map | Stale after quota-blocked refresh | `graphify-out/GRAPH_REPORT.md` and `graphify-out/graph.json` were last successfully regenerated through source commit `75f5345d`. A post-`b69b638` refresh attempt scanned 595 code files and 257 docs but all 7 OpenAI semantic chunks failed with `insufficient_quota`; the tool still wrote partial output, so it was discarded. Local Ollama/Gemma completed a semantic smoke test, but a full-repo attempt hit repeated context splits and invalid JSON chunks before it was stopped. | Restore OpenAI quota or validate a more reliable approved local/offline Graphify backend, then rerun Graphify before major cleanup. |
...
2. Fresh Graphify and architecture inventory.
   ...
   - Latest blocked OpenAI attempt after `b69b638` scanned 595 code files and 257 docs; all 7 semantic chunks failed, and the partial graph output was discarded.
