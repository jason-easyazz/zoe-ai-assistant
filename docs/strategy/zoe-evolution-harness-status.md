...
- Graphify was last successfully refreshed through source commit `75f5345d`.
  ...
  must be treated as stale for changes after `75f5345d`.
...
2. Fresh Graphify and architecture inventory.
   - Status: stale after source commit `75f5345d`; later refresh attempts are blocked by OpenAI `insufficient_quota`.
   - Latest blocked attempt after `b69b638` scanned 595 code files and 257 docs; all 7 semantic chunks failed, and the partial graph output was discarded.
