...
- The Hindsight/Graphiti bake-off harness (synthetic fixtures, scoring helpers, read-only probes) concluded and was retired by removal on 2026-07-03; measured results remain in `docs/architecture/zoe-hindsight-bakeoff.md` and `docs/architecture/zoe-graphiti-fixtures.md`.
...
- Pi hybrid production is live for the touch-panel voice path: instant processing cue, standalone Pi RPC classification, and safe Zoe fulfillment for `weather` and `daily_briefing` are enabled in production.
- Pi promotion evidence now has a closed production loop: live decisions write sanitized JSONL evidence, admin labels can attach outcome and comparable baseline fields, readiness scoring consumes the labeled production samples, and PR #648 constrained the label vocabulary.
...
- Graphiti/FalkorDB/Neo4j have not yet been accepted for Zoe relational memory. Zoe-specific Graphiti fixtures, read-only backend/runtime/local-model probes, and an inert runtime-trial proposal builder now define the first relationship eval set, availability preflight, structured-output gate, and governed optional-runtime request. A temporary FalkorDB sidecar was reachable, Zoe's normal Python runtime lacks Graphiti backend packages, Graphiti's library extractor failed an earlier local structured-output smoke test, and the explicit local Gemma contract probe now passes at about 6.1 s.
...
- Zoe now has an inert Multica handoff packet builder, runtime-callable
  ...
  memory candidates and capability trust evidence. The outcome workflow is
  plan-only by default, but it also has an explicit `--execute-hindsight` mode
  that calls the admitted Hindsight retain executor only when approval evidence
  admits the plan; pending, blocked, disabled, and config-error paths return
  structured non-retain results and are covered by tests.
...
- Zoe now has a read-only Pi runtime probe, policy contract, and inert runtime-adoption proposal builder; actual Pi execution is still blocked until Node/npm/Pi and local/offline model configuration are present and approved.
...
- Retain-candidate admission has a tested contract, and Hindsight retain
  ...
  admission decisions. The profile-edit outcome operator can opt into that same
  admitted executor with `--execute-hindsight`, while production chat still does
  not auto-write them.
...
| Hindsight bake-off | Concluded; harness retired by removal 2026-07-03 | Offline sidecar client, retain-candidate helpers, availability doc, admission bridge, admitted retain-plan gate, admitted retain executor, and their tests remain in the tree; the bake-off harness (synthetic fixtures, measured runner, read-only probes) was retired by removal and lives in git history. First live offline sidecar retained 4/4 synthetic events with 0 extraction errors and recalled 4/4 cases at score 1.0 using local Gemma plus cached local BGE embeddings. Live p95 recall was 649.00 ms and warm p95 was 643.25 ms. A later low/mid budget matrix retained 4/4 synthetic events, recalled 8/8 cases at score 1.0, and measured aggregate p95 657.52 ms, low p95 647.14 ms, and mid p95 637.34 ms; the sidecar sampled about 936.9 MiB at cleanup. | Treat Hindsight as validated for offline experience recall but not accepted for hot-path prompt recall; the bake-off harness is retired — re-derive from git history only if a future measured trial is approved. |
| Graphiti bake-off | Concluded; harness retired by removal 2026-07-03 | The probe chain (bake-off fixtures, sidecar/runtime/local-model probes, inert proposal driver) and its tests were retired by removal after the bake-off concluded; `docs/architecture/zoe-graphiti-fixtures.md` and the ADR keep the measured record (FalkorDB reachable in a temporary sidecar, runtime lacked Graphiti backend packages, library extractor failed local structured output, explicit local Gemma contract probe ~6.1 s). The Samantha build plan mandates no heavy graph DB on this host. | None — Graphiti was not adopted; re-derive the harness from git history only if a future approved trial changes the no-graph-DB constraint. |
| Graphify current map | Stale full map with current accepted default shards | `graphify-out/GRAPH_REPORT.md` and `graphify-out/graph.json` were last successfully regenerated through source commit `75f5345d`. A post-`18b8bd5` OpenAI refresh attempt scanned 599 code files and 257 docs but all 7 semantic chunks failed with `insufficient_quota`; the partial output was discarded. A post-`015a529` local Gemma attempt reached AST extraction for 601 code files and 257 docs through Zoe's localhost llama.cpp server, then was stopped after repeated context splits and three invalid JSON chunks; no partial graph output was committed. `scripts/maintenance/graphify_local_probe.py` now provides an offline/local probe that strips cloud keys, runs in a temporary fixture or snapshot, and fails closed on invalid JSON, truncation, missing graph output, or quota signals. The former sync-capable local wrapper (`scripts/maintenance/graphify_local_refresh.py`) has since been removed with Graphify's retirement; `graphify-out/` is no longer committed and nothing syncs graph output back into the repo. `scripts/maintenance/graphify_shard_sync_plan.py` now converts accepted shard-matrix evidence into a no-write merge design plan, and `graphify_local_shard_matrix.py --artifact-dir` can copy accepted per-shard `graphify-out` artifacts outside the repo while keeping `artifact_sync_ready=false` until deterministic graph merge and report generation are proven. The Graphify refresh timer has been retired entirely (no script writes `graphify-out/` into the repo). A post-`5c9ef0f` repo-mode probe scanned 603 code files and 257 docs, completed AST extraction, then rejected the run after a 300 second timeout, 11 context split warnings, three invalid JSON chunks, and one truncated chunk; temporary graph output was not accepted or committed. A later local model matrix accepted Gemma 4 E2B, E4B, and 12B for scoped `services/zoe-data` extraction with 290 code files, 10 docs, no invalid JSON/truncation/context splits, and E4B fastest at 47.087 s. A full-repo E4B local refresh after `3dd3218` still rejected after the 1800 second timeout with AST extraction complete over 617 code files and 257 docs, temporary graph output present, 58 context splits, 26 invalid JSON chunks, and 6 truncated chunks; the wrapper did not sync partial `graphify-out` output. Current default shard matrix evidence against `origin/main` at `2032b40` accepted 2/2 bounded shards without `--allow-partial`: `data-core` in 43.524 s over 291 code files and 10 docs with 5,181 nodes and 11,355 edges, and `operators` in 49.883 s over 181 code files and 9 docs with 1,652 nodes and 3,121 edges; no invalid JSON, truncation, or context splits, median accepted duration 46.704 s, and max observed child RSS 86,408 KB. A later artifact-preserving run copied both accepted shard `graph.json` files outside the repo and the planner reported `artifact_capture_ready=true`, `artifact_report_ready=false`, and `artifact_sync_ready=false`. | Add deterministic graph JSON merge validation over the captured shard artifacts, then prove merged cluster/report generation and inventory reconciliation before replacing the stale committed full map. |
...
| Pi runtime harness | Partial with production hybrid lane | `services/zoe-data/pi_runtime_probe.py`, `scripts/maintenance/pi_runtime_probe.py`, `scripts/maintenance/pi_runtime_proposal.py`, `services/zoe-data/zoe_evolution_runtime_intake.py`, tests, and `docs/architecture/zoe-pi-runtime-harness.md` still define runtime readiness and proposal gates. Separately, the deployed production hybrid lane now uses standalone Pi RPC for narrow low-risk intent confirmation behind an instant voice cue, while Zoe owns fulfillment and fallback. `/api/system/status` reports `pi_hybrid_production.ok=true`; readiness reports `ready=true`, `mode=shadow_buffer`, `production_groups=[daily_briefing, weather]`, 8 production records, 2 production labels, and no unlabeled production rows after PR #648. | Keep hybrid production scoped to safe groups until formal promotion gates clear. Next collect real/log-derived labeled evidence for candidate groups and promote only if speed/accuracy policy passes. |
...
| Memory admission governance | Complete foundation | `services/zoe-data/zoe_memory_admission.py`, `services/zoe-data/hindsight_retain_candidates.py`, `services/zoe-data/hindsight_retain_executor.py`, `services/zoe-data/zoe_multica_memory_admission.py`, `services/zoe-data/zoe_evolution_outcome_admission.py`, `services/zoe-data/zoe_evolution_outcome_retain.py`, `services/zoe-data/zoe_capability_profile_edit_outcome.py`, `scripts/maintenance/capability_profile_edit_outcome_workflow.py`, tests, and `docs/architecture/zoe-memory-admission-gates.md` define evidence, trace, approval, graph, Multica review metadata, outcome memory admission, Hindsight admitted retain planning/execution, admitted outcome retain execution, profile-edit outcome admission, and self-evolution proposal gates before durable memory writes; Hindsight retain candidates, Multica review records, terminal evolution outcomes, and verified profile-edit outcomes can now build/evaluate admission requests before promotion, Hindsight candidate metadata reaches MemoryService writer metadata with first-class scope/evidence/relationship fields, `--execute-hindsight` can execute an already admitted profile-edit outcome retain plan on operator request, and `admit_hindsight_retain_candidate(event_id)` gives future runtime code an inert no-write pending-review entrypoint instead of direct sidecar retain calls. | Route future MemPalace, Graphiti, and non-Hindsight outcome-memory durable writers through admission decisions before any backend write. |
...
2. Fresh Graphify and architecture inventory.
   ...
   - Use captured shard artifacts plus the no-write sync plan (`artifact_capture_ready=true`, `artifact_sync_ready=false`) to add deterministic merge validation before treating Graphify as current for major cleanup.
...
6. Graphiti relational bake-off.
   ...
   - Evidence: FalkorDB can be reachable as a local sidecar, but Zoe's normal Python runtime lacks Graphiti packages; local Gemma passes the explicit no-write structured-output probe at about 6.1 s, while Graphiti's own extractor still needs compatibility proof before ingest/query acceptance.
   - Next: review the inert optional-runtime proposal, approve local dependencies/sidecar work only with operator evidence, then solve structured-output compatibility before FalkorDB fixture ingest/query measurements; test Neo4j only if feasible.
...
12. Pi runtime harness and hybrid production.
    - Status: runtime probe/proposal foundation remains, and production now has a narrow hybrid lane for touch-panel voice.
    - Current production hybrid evidence: instant cue plus Pi RPC plus safe Zoe fulfillment is enabled for `weather` and `daily_briefing`; `/api/system/status` reports `pi_hybrid_production.ok=true`; readiness reports `ready=true`, `mode=shadow_buffer`.
    - Formal promotion is not blocked by configuration; it is blocked by evidence volume. Candidate wins exist, but each group still needs 30 unique real/log-derived labeled samples before default routing promotion.
    - Next: collect and label real production/shadow evidence for candidate groups, then apply promotion only when the speed/accuracy policy clears.
