...
### Merge order — this change is gated by a SEPARATE, earlier PR

`functiongemma-router.service` was not in `VOICE_PATH_PATTERNS` when this work
started, so the change below is a **voice-path change that the gate could not
see**. It is not fixed by adding the pattern here, and that is worth stating
plainly because the first attempt did exactly that and was wrong:

**A new pattern cannot gate the PR that adds it.** `voice-gate.yml` triggers on
`pull_request_target` and pins `base.sha` on every checkout — deliberately, so an
untrusted PR cannot author its own verdict — which means the classifier is always
the **base ref's** copy of `voice_gate_check.py`. Patterns added by a PR do not
exist for the run classifying it. Measured on the superseded #1620, whose head
added three router patterns: run 30804036970 reported `scope=non-voice`,
`replay-evidence=SKIPPED`. It silently skipped the gate it was extending.

So the coverage lands FIRST, in `fix/voice-gate-router-coverage` (checker + tests
+ docs, itself correctly non-voice). Once that is on `main`, a PR carrying the
unit change below is classified by a `main` that already knows the unit is on the
voice path, and the replay evidence is genuinely required rather than nominally
claimed. Do not merge the caps change ahead of the gate change; doing so puts a
voice-path edit on `main` with the gate structurally unable to see it.
