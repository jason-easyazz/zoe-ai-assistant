...
- [Production incident runbook](incident-runbook.md) — verified failure signatures + fixes: the zoe-data accept-queue hang (health 000, service "active") and root-owned lab-container files blocking every deploy; prevention rules for lab containers. Also: the voice stack swapped out — replies chopped + /health timing out while everything reports healthy (2026-07-19); the brain's CUDA-OOM crash-loop under Tegra unified-memory pressure — replay gate collapses with ~100 ms brain_ms connection-refuseds while tier0 stays green (2026-07-19); and `MemoryMax` without `MemorySwapMax=0` being no cap at all — MemoryHigh pressure overflows into unbounded swap (2026-07-20).
