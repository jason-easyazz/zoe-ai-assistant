...
| 25 | Web search / research | `research` delegation seam (`/api/system/delegate-sync`); legacy `web_search` tool | `ZOE_SEARCH_PROVIDER` default `auto`, `TAVILY_API_KEY` **set**, `cloakbrowser` installed | **DARK** (seam not restored; claim still advertised) | The live Flue 2.x brain registers 20 tools (`add_reminder, add_to_list, get_weather, home, media, people, recall_memory, …`) and — **by the approved cut list** ([flue-cutover-tool-cut-list.md](flue-cutover-tool-cut-list.md) item 10) — no direct `web_search`; the replacement is the `research` ability routed through `delegate-sync`, whose target (Hermes) is inactive and was to be re-pointed to Flue. `chat.py` still binds the browser broker to the OpenClaw gateway (`:18789`, offline). So "are you sure? / look it up" cannot reach the web today, while `chat/capabilities` still advertises it. PR #1610 (spike) is stale/conflicting (H9). |
...
1. `proactive/triggers/reminder_scan.py:13` — *"Reminder has no due_time → skip"*. All five
   active reminders (created 07-06 … 08-10, due dates in July/August) are **date-only**, so
   the scan (which does run every 5 min) schedules nothing, and `proactive_scheduled` shows
   no fire for `jason` after 2026-07-04.
2. The voice path stored the literal string **`"tomorrow"`** as `due_date` for one reminder
   (with a valid `due_time`); `build_run_at()` does `date.fromisoformat("tomorrow")` →
...
- the four date-only rows with past dates: either **acknowledge** (`POST /acknowledge`) or
  **re-date** (`PUT` with a new `due_date` + `due_time`) — operator's call per row; the
  reminder scan then picks up any re-dated one on its next 5-minute pass;
- the `"tomorrow"` row stays broken until it is repaired via `PUT` with an ISO date or
  deleted; nothing in the scan will ever coerce it.
...
### 2.6 Web search / research (DARK — a claim without a working seam)
...
`web_search_provider.py`), but nothing on a live turn can reach them. This is **not** a
missing tool: the approved cut list
...
validation layer in `delegate-sync`. What is broken is that seam: its target (Hermes) is
inactive and the planned re-point to Flue (cut-list items 8/15) has not landed, and the
browser broker still targets the retired OpenClaw gateway.
**Fix, in the approved direction:** either restore the `research` → `delegate-sync` seam
with a Flue-backed target (per the cut list; do **not** add a direct `web_search` tool to
the brain), or — until that lands — remove the stale capability claim from the
`chat/capabilities` text so the brain stops promising it. The memory note
`project_zoe_web_search_browsing` still wants the CloakBrowser-vs-search-API recommendation
before the seam is rebuilt.
