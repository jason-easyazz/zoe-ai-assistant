...
- **`omp config set` fencing is defeatable by the agent itself**: a `<cwd>/.omp/config.yml` written by a misbehaving agent re-enabled web_search/autoqa/autoUpdate in a measured test. The fence is a **`PI_CONFIG_FILES` overlay** (fail-closed on missing file) — all six fence values held against the hostile cwd config in that test. **It outranks the cwd config, but it is NOT the top of the stack, and two conditions are load-bearing** (verified in source, not assumed):
  - **The overlay path MUST be absolute.** `Settings`' constructor does `path.resolve(this.#cwd, expandTilde(file))` (`packages/coding-agent/src/config/settings.ts:383`) — a *relative* entry resolves against the agent's own working folder, i.e. exactly the cwd the hostile-config test proved an agent can write. A relative fence path would let the agent supply the fence. Keep it absolute and **outside the dispatch working folder**.
  - **No later `--config` overlay or runtime override may be passed.** `PI_CONFIG_FILES` entries load *before* explicit `--config` overlays (`docs/settings.md:125`, `docs/environment-variables.md:366`), and arrays/scalars are replaced wholesale by the higher layer — so a subsequent `--config` re-enables `web_search`/AutoQA/`autoUpdate` over the fence. The wrapper must therefore own the full argv and forbid pass-through of `--config` and setting overrides.

  The `config get` read-backs verify the values *as launched by the wrapper*; they cannot detect an overlay a later argv or a relative-path resolution would introduce. Both conditions belong in the wrapper's own assertions before first dispatch.
