...
- Do **not** wire any lab code into the live voice path, `zoe-data`, a systemd
  ...
  *template* — `scripts/setup/systemd/flue-zoe-brain.service`, port 3578 — that is
  ...
  through zoe-data's `ZOE_BRAIN_BACKEND=flue` seam. The **shipped repo default is
  OFF** (`core` = `services/zoe-core`), but the seam is production-reachable and
  **this deployment flipped it live on 2026-07-03** — so `flue-zoe-brain/` is
  lab-hosted yet production-reachable, not "never live". No other lab may ship a
  unit without amending this contract.)
