 ...
 *
 * FLUE 2.x TOOL CONTRACT. `defineTool({ name, description, input, output, run })`
 * is unchanged, but two things moved and one of them fails at RUNTIME rather
 * than compile time:
 *   - `run({ input })` → `run({ data })`. The parsed-arguments field on
 *     `ToolContext` is now `data` (and it is the schema's OUTPUT type, so a
 *     `v.pipe(v.string(), v.trim())` arrives already trimmed). `signal` is
 *     unchanged — which is what the per-turn identity binding rides on — and
 *     `log` / `toolCallId` are now always present.
 *   - `run()` returns a result envelope `{ output?, terminate? }`. A bare
 *     `string` return is still legal sugar for `{ output: <string> }`, but any
 *     OTHER bare value (object, number, boolean, array, null) now THROWS at
 *     runtime. EVERY tool below returns a string, so none needed wrapping — but
...
//
// These were module-load constants. They are now functions, for the same reason
// every other env read in this sidecar is (`maxToolIters`, `brainTemperature`,
// `contextWindowTokens`, `progressiveToolsEnabled` all say "read fresh each call
// — no module-load pinning"): this file was the one place that violated the
// convention. Module-load pinning meant an operator changing ZOE_DATA_URL or
// ZOE_BRAIN_ALLOW_WRITES needed a restart, and — the reason it surfaced — the
// values were captured at IMPORT time, before a test could point the tools
// anywhere safe. A tool suite that silently addresses the LIVE zoe-data on
// :8000 is not a lab suite. Same idiom, same validation, no behaviour change for
// a normally-configured process.

/** zoe-data base URL — the live capability backend. Defaults to prod's local endpoint. */
function zoeDataUrl(): string {
  return process.env.ZOE_DATA_URL ?? 'http://127.0.0.1:8000';
}

function internalToken(): string {
  return process.env.ZOE_INTERNAL_TOKEN ?? '';
}

/**
 * Per-tool HTTP timeout. Validated: Number('') is 0 and Number('abc') is NaN,
 * either of which would make AbortSignal.timeout() abort every call immediately.
 * Fall back to 8s for any non-positive / non-finite value.
 */
...
// Writes are off by default in the lab: every write tool becomes a DRY-RUN
// unless explicitly enabled, so the parity harness can't mutate the real data.
function allowWrites(): boolean {
  return (process.env.ZOE_BRAIN_ALLOW_WRITES ?? 'false').toLowerCase() === 'true';
}
...
async function dispatchIntent(
  ...
  try {
    const url = new URL('/api/system/intent-dispatch', zoeDataUrl());
...
const recallMemory = defineTool({
  ...
  run: async ({ data, signal }) => {
    ...
    try {
      const url = new URL('/api/memories/for-prompt', zoeDataUrl());
      ...
      // Named `body`, not `data`: the 2.x ToolContext argument is now `data`, so
      // a local of that name would shadow the tool's own parsed arguments.
      const body = (await res.json()) as { packet?: string };
      const packet = (body.packet ?? '').trim();
...
const getWeather = defineTool({
  ...
  run: async ({ data, signal }) => {
    const slots: Record<string, unknown> = { forecast: Boolean(data?.forecast) };
...
const showList = defineTool({
  ...
  run: async ({ data, signal }) => {
    const listType = normalizeListType(data?.list_type);
...
const setTimer = defineTool({
  ...
  run: async ({ data, signal }) => {
    ...
    const label = String(data?.label ?? '').trim() || 'Timer';
...
const createNote = defineTool({
  ...
  run: async ({ data, signal }) => {
    ...
    const title = String(data?.title ?? '').trim() || content.slice(0, 60);
...
const journal = defineTool({
  ...
  run: async ({ data, signal }) => {
    if (data.action === 'prompt' || data.action === 'streak') {
      const intent = data.action === 'prompt' ? 'journal_prompt' : 'journal_streak';
...
const people = defineTool({
  ...
  run: async ({ data, signal }) => {
    if (data.action === 'search') {
      const query = String(data?.query ?? '').trim() || String(data?.name ?? '').trim();
    ...
    const slots: Record<string, unknown> = {
      name,
      relationship: String(data?.relationship ?? '').trim() || 'friend',
...
const media = defineTool({
  ...
  run: async ({ data, signal }) => {
    switch (data.action) {
      case 'play': {
        const query = String(data?.query ?? '').trim() || 'music';
      ...
      case 'set_music_volume': {
        if (data?.level == null) return 'What volume would you like (0-100)?';
      ...
      case 'system_volume': {
        const direction = String(data?.direction ?? 'up').toLowerCase();
        if (direction === 'set') {
          if (data?.level == null) return 'What level should I set my speaking volume to (0-100)?';
          const level = clampLevel(data.level);
...
const home = defineTool({
  ...
  run: async ({ data, signal }) => {
    const action = ACTION_MAP[data.action];
    const roomRaw = String(data?.room ?? '').trim();
    const room = roomRaw ? roomRaw.toLowerCase().replace(/\s+/g, '_') : '';
    const where = room ? `the ${roomRaw} lights` : 'the lights';
    const label = { on: 'on', off: 'off', dim: 'dimmed', brighten: 'brightened' }[data.action];
    ...
    return runWrite('smart_home', slots, 'smart home',
      `turning ${where} ${data.action}`, `Turned ${where} ${label}.`, signal);
  },
});
...
const rememberEmotionalMoment = defineTool({
  ...
  run: async ({ data, signal }) => {
    const moment = String(data.moment ?? '').trim();
...
const activateAbilities = defineTool({
  ...
  run: async ({ data }) => {
    const tools = TOOL_GROUPS[data.group];
    return (
      `Activated the ${data.group} tools: ${tools.join(', ')}. ` +
