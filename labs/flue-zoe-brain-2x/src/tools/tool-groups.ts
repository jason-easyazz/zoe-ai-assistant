 ...
 * never mutated).
 *
 * `basis` is the message list the ACTIVE SET is derived from; it defaults to
 * `context.messages`. The caller (`applyPolicies`) passes the PRE-WINDOW
 * messages, which is load-bearing for two reasons:
 *   - CORRECTNESS: sources 3 and 4 above (explicit activations, sticky used
 *     groups) read the transcript. Deriving them from the WINDOWED list made a
 *     group silently retract once the tool call that activated it aged out of
 *     the prompt window, contradicting this module's documented "the per-session
 *     set grows monotonically" contract.
 *   - PROMPT STABILITY: a retracting group re-renders the tool block, and the
 *     tool block sits in the prompt prefix ahead of every message. A
 *     non-monotone active set therefore oscillated the prefix between rounds and
 *     cost a full llama-server re-prefill each time.
 * Deriving from the full basis makes the set monotone in the session, so the
 * rendered tool block only ever grows — a stable prefix by construction.
 *
 * Rules, in order:
