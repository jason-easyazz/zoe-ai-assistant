...
def assert_dry_run() -> None:
    ...
    try:
        flue_wire.ask(sid, "show me my shopping list")
...
def main() -> None:
    ...
    for expected, prompt in PROMPTS:
        for rep in range(REPEATS):
            ...
            # PORTED TO THE 2.x WIRE. The beta did POST ?wait=result for the text,
            # then a second GET of the event stream for the authoritative tool list.
            # 2.x rejects ?wait=result outright, and the NDJSON stream carries BOTH:
            # the reply text and the __TOOL__ phase=start sentinels. One request,
            # same authoritative signal (the sentinels are emitted from the very
            # tool events the old GET was reading), and no polling race between the
            # POST returning and the events being persisted.
            text, sentinels, ms = flue_wire.ask(sid, prompt)
            called = flue_wire.tool_names(sentinels)
