...
# The Flue sidecar URL now comes from flue_wire (ZOE_BRAIN_URL), because the 2.x
# wire has no synchronous "?wait=result" call left to hard-code — see parity/flue_wire.py.
...
def ask_flue(prompt: str, sid: str) -> tuple[str, float]:
    # PORTED TO THE 2.x WIRE: fire-and-forget admission + stream read. The beta
    # `?wait=result` call is actively REJECTED by 2.x, not just ignored.
    text, _sentinels, ms = flue_wire.ask(sid, prompt)
