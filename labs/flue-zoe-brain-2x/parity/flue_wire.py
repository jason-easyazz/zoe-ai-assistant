...
def base_url() -> str:
    ...
    Defaults to :3579 — `flue-zoe-brain-2x.service`, which since the 2026-08-09
    cutover is the SOLE LIVE Zoe brain. So the default aims at the live brain: to
    probe in isolation, start a hand instance on a spare port and set
    ``ZOE_BRAIN_URL`` to it, exactly as the LANDING.md checklists do. The retired
    1.x beta (:3578, `@flue/*@1.0.0-beta.6`) and its source were removed in #1678;
    there is no longer a separate lab port to fall back to.
    """
