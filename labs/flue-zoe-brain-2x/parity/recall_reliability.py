...
def main() -> None:
    ...
    for i, prompt in enumerate(PROMPTS):
        ...
        try:
            text, sentinels, _ms = flue_wire.ask(sid, prompt)  # PORTED: 2.x wire
        except Exception as exc:  # noqa: BLE001
            rows.append((prompt, "ERR", f"post failed: {exc}", set()))
            print(f"[{i+1:2d}/{n}]  ERR   {prompt!r}  ({exc})")
            continue
        # PORTED TO THE 2.x WIRE. The beta unwrapped `{"result": {"text": ...}}`
        # from the ?wait=result response, then slept 0.3s and re-fetched the
        # persisted event log to see which tools fired. Both are gone: 2.x has no
        # whole-result response, and the NDJSON stream already carries the reply
        # text plus the __TOOL__ sentinels. Dropping the sleep+refetch also drops
        # a real flake source — the old code scored a MISS whenever the event log
        # had not settled within 300ms.
        called = flue_wire.tool_names(sentinels)
        ok = "recall_memory" in called
        tools = set(called)
        if ok:
            fired += 1
