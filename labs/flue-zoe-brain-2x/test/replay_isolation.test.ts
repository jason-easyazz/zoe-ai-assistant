/**
 * Replay write-isolation — the sidecar half, 2.x port.
 *
 * Mirrors labs/flue-zoe-brain/test/replay_isolation.test.ts. Adapted for the 2.x
 * tool contract: `run({ data, signal })`, not `run({ input, signal })`.
 *
 ...
 * that.
 *
 * The expectations are the 2.x tools' own `successFallback` literals, re-read from
 * this port's src/tools/zoe-tools.ts rather than inherited from 1.x.
 */
...
async function withWriteEnabledTools(baseUrl: string, fn: (tools: RunnableTool[]) => Promise<void>) {
  const prev = {
    ...
    timeout: process.env.ZOE_BRAIN_TOOL_TIMEOUT_MS,
  };
  ...
  try {
    // 2.x DRIFT: this port reads ZOE_DATA_URL / ZOE_BRAIN_ALLOW_WRITES fresh on
    // every call (zoeDataUrl(), allowWrites()), so a fresh module instance is not
    // strictly required the way it is on 1.x. The distinct-URL import is kept
    // anyway — it costs nothing and keeps this file insensitive to whether some
    // OTHER test in the same process already imported the module under different
    // env. Node's ESM resolver caches by full URL, so a distinct query string
    // yields a distinct instance (https://nodejs.org/api/esm.html#urls).
    ...
    for (const [k, v] of Object.entries({
      ZOE_DATA_URL: prev.url,
      ZOE_BRAIN_USER_ID: prev.uid,
      ZOE_BRAIN_ALLOW_WRITES: prev.writes,
      ZOE_BRAIN_TOOL_TIMEOUT_MS: prev.timeout,
    })) {
  ...
  try {
    await withWriteEnabledTools(fake.baseUrl, async (tools) => {
      for (const c of WRITE_CASES) {
        ...
        await byName(tools, c.name).run({ data: c.input, signal });
      }
      assert.equal(
  ...
  try {
    await withWriteEnabledTools(fake.baseUrl, async (tools) => {
      for (const c of WRITE_CASES) {
        ...
        const out = String(await byName(tools, c.name).run({ data: c.input, signal }));
...
test('an identity-less replay turn keeps the fail-closed line and still writes nothing', async () => {
  ...
  try {
    await withWriteEnabledTools(fake.baseUrl, async (tools) => {
      ...
      const signal = new AbortController().signal;
      bindTurnReplayMode(signal, true);
      const out = String(await byName(tools, 'add_reminder').run({ data: { title: 'x' }, signal }));
...
test('reads are unaffected by the replay marker (the gate still scores said-vs-did)', async () => {
  ...
  try {
    await withWriteEnabledTools(fake.baseUrl, async (tools) => {
      const signal = new AbortController().signal;
      bindTurnReplayMode(signal, true);
      await byName(tools, 'show_calendar').run({ data: {}, signal });
