 ...
 *
 *  1. AUTH FAIL-CLOSED — `requireBrainToken()` (mounted on a real Hono app) rejects
 *     an unauthorized caller and never runs the downstream handler; a matching
 *     bearer (or ZOE_BRAIN_OPEN=1) lets it through. The gate never threads
 *     identity itself — it only gates access.
 *
 *     PORTED FOR FLUE 2.x: this used to import `route` from the agent module.
 *     That `export const route` convention is deleted in 2.x; the same gate is
 *     now ordinary Hono middleware in src/auth.ts. The ASSERTIONS are unchanged
 *     on purpose — this is the security boundary the whole forwarded-identity
 *     trust model rests on, so the port must satisfy the same contract the beta
 *     did, not a re-negotiated one. test/auth_gate.test.ts adds the cases the
 *     beta suite never covered (unconfigured token, wrong scheme, wrong token).
 *
 ...
 *                         →  tool.run({ data, signal })      (acts as that user)
...
const { requireBrainToken } = await import('../src/auth.ts');
...
function appRan(seen: { ran: boolean }): Hono {
  const app = new Hono();
  app.post('/agents/zoe/:id', requireBrainToken(), (c) => {
    seen.ran = true;
...
test('chain: provider binds the enveloped user by signal; the tool acts as THAT user', async () => {
  await withCapturedFetch(async (getUrl) => {
    ...
    const out = await recallMemory.run({ data: {}, signal });
...
test('chain: two concurrent turns stay isolated (no cross-user leak)', async () => {
  await withCapturedFetch(async () => {
    ...
    const outA = await recallMemory.run({ data: {}, signal: sigA });
    const outB = await recallMemory.run({ data: {}, signal: sigB });
