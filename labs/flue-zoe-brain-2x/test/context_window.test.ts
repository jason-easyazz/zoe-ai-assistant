...
const { applyPolicies, createZoeProvider } = await import(
...
test('over-budget stray preamble before the first user turn is dropped', () => {
  ...
  //
  // PORTED FOR THE QUANTISED ANCHOR (2026-08-03). The beta dropped the MINIMUM
  // needed, so it retained the 'hello' block here. The anchor is now quantised —
  // when the head must move it moves by at least one quantum (~25% of budget) so
  // it then holds still for many turns instead of creeping forward every turn
  // and cold-prefilling the whole prompt each time. A bounded amount of extra
  // history is dropped at each re-anchor; that is the trade, and this degenerate
  // case (one huge preamble, two tiny turns) shows its worst shape.
  //
  // So this test now asserts the INVARIANTS that are actually contractual rather
  // than the old algorithm's incidental minimality: the result fits budget,
  // starts at a user message, keeps the newest turn whole, and the preamble is
  // gone. Anchor stability itself is covered by test/context_window_anchor.test.ts.
  ...
  assert.equal(
    windowed.messages[windowed.messages.length - 1].content,
    'bye',
    'the newest turn must always survive',
  );
  assert.ok(
    !windowed.messages.some((m) => String(m.content).startsWith('ppp')),
    'the over-budget preamble must be gone',
  );
});
...
/**
 * PORTED FOR FLUE 2.x. The beta reached the capped handler through pi-ai's
 * GLOBAL api registry: `registerApiProvider({ api: 'zoe-capped-completions',
 * ... })` seated it under a slug, a hand-built model carried `api: <slug>`, and
 * the root `completeSimple()` dispatched on that slug. All three are gone —
 * `registerApiProvider` does not exist on 2.x, the registry with it, and
 * `completeSimple` moved to the `./compat` subpath.
 *
 * The replacement is strictly better for this test: the provider OWNS its
 * `{ stream, streamSimple }` pair, so driving the real `createZoeProvider()`
 * through a `Models` collection exercises the actual production path end to end
 * — provider construction, model metadata, auth resolution (`auth.apiKey
 * .resolve()`), and the capped wire handler — instead of a hand-assembled model
 * pointed at a globally-registered slug. Nothing about the assertions changes.
 */
async function zoeModels(baseUrl: string) {
  const { createModels } = await import('@earendil-works/pi-ai');
  const saved = process.env.ZOE_BRAIN_BASE_URL;
  process.env.ZOE_BRAIN_BASE_URL = baseUrl;
  try {
    const models = createModels();
    models.setProvider(createZoeProvider());
    const model = models.getModel('zoe', 'local');
    assert.ok(model, 'the zoe provider did not publish its local model');
    return { models, model };
  } finally {
    if (saved === undefined) delete process.env.ZOE_BRAIN_BASE_URL;
    else process.env.ZOE_BRAIN_BASE_URL = saved;
  }
}
...
test('an over-budget stored session still answers end-to-end through the capped provider', async () => {
  ...
  try {
    const { models, model } = await zoeModels(fake.baseUrl);
    ...
    process.env.ZOE_BRAIN_CONTEXT_WINDOW = '0';
    const wedged = await models.completeSimple(model, context);
    ...
    resetEnv();
    const answered = await models.completeSimple(model, context);
