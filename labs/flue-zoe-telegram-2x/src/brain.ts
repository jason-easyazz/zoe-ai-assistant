 ...
 *
 * FLUE 2.x DELTA: NONE. This module is `fetch` + `node:fs` only and touches no
 * Flue API, so it ports across the 2.x boundary unchanged apart from where the
 * epoch file is anchored (below). That is itself the finding: the four zoe-data
...
//
// THIS FILE IS OURS, NOT FLUE'S — and that distinction is load-bearing at
// cutover. Flue's own conversation store (src/db.ts) cannot cross the 1.x→2.x
// schema boundary in either direction; this epoch map is plain JSON we wrote, so
// it copies over verbatim and users keep their `/new` history. See README.md.
import { mkdirSync, readFileSync, renameSync, writeFileSync } from 'node:fs';
import { dirname, join } from 'node:path';
import { fileURLToPath } from 'node:url';

// Anchor the default at the PACKAGE ROOT (the parent of the built `dist/`),
// not the process cwd. Under the systemd unit (WorkingDirectory=<package>) this
// is byte-identical to the 1.x `./data/session_epochs.json`; run by hand from
// anywhere else it now resolves to the same file instead of silently starting a
// second, empty epoch map beside wherever you happened to be. Same class of bug
// as the brain port's `dist/data/...` finding.
const PACKAGE_DATA_DIR = join(dirname(fileURLToPath(import.meta.url)), '..', 'data');

// Read at CALL time (not module load) so tests can point it at a temp file.
function epochsPath(): string {
  return process.env.SESSION_EPOCHS_PATH ?? join(PACKAGE_DATA_DIR, 'session_epochs.json');
}
