 ...
 * not.
 *
 * WHY NOT AN ENV FLIP — `ALLOW_WRITES` in src/tools/zoe-tools.ts is a module-LOAD
 * const, so flipping `ZOE_BRAIN_ALLOW_WRITES` needs a sidecar restart in both
 * directions. A probe that stops the live brain, flips, replays and flips back
 * wedges the voice lane if it crashes mid-run. And `ZOE_BRAIN_ALLOW_WRITES=false`
 * is NOT scorer-neutral: its dry-run text instructs the model to say "you can't do
 * that yet", which `replay_samples.py::_CANT_DO_RE` scores CANT_DO — turning the
 * env off would redden the gate on every write command in the corpus. That is
 * precisely why the .env carries `true`, and why the fix has to be per-REQUEST.
 *
