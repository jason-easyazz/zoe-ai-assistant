...
class PersonalWidget extends WidgetModule {
    ...
    async loadItems() {
        try {
            // GET /api/lists/personal_todos returns { lists: [...] } with no
            // items inline, so we must fetch the first list's detail.
            const listsResponse = await apiRequest(`/api/lists/personal_todos`);
            ...
            const detail = await apiRequest(`/api/lists/personal_todos/${this.listId}`);
    ...
    async ensureList() {
        ...
        try {
            const response = await apiRequest('/api/lists/personal_todos', {
                method: 'POST',
                body: JSON.stringify({
                    name: 'Personal Todos',
    ...
    async addItem(text) {
        ...
        try {
            await apiRequest(`/api/lists/personal_todos/${listId}/items`, {
                method: 'POST',
    ...
    async toggleItem(itemId) {
        ...
        try {
            await apiRequest(`/api/lists/personal_todos/${this.listId}/items/${itemId}`, {
                method: 'PUT',
                body: JSON.stringify({ completed: !item.completed })
            });
            await this.loadItems();
        } catch (error) {
            console.error('Failed to toggle personal item:', error);
        }
    }
    ...
    async archiveOldCompletedItems() {
        ...
        const itemsToArchive = this.items.filter(item => {
            ...
            // completedAt may not be set on server-managed items; fall back to
            // updated_at which the backend maintains.
        ...
        if (this.listId) {
            for (const item of itemsToArchive) {
                try {
                    await apiRequest(`/api/lists/personal_todos/${this.listId}/items/${item.id}`, {
                        method: 'DELETE'
                    });
                } catch (err) {
                    console.warn('Failed to delete archived personal item on server:', err);
                }
            }
        }
