...
class WorkWidget extends WidgetModule {
    ...
    async loadItems() {
        try {
            const response = await apiRequest(`/lists/work_todos?user_id=${this.userId}`);
    ...
    async saveItems() {
        try {
            ...
            if (existingLists.length > 0) {
                ...
                await apiRequest(`/lists/work_todos/${listId}?user_id=${this.userId}`, {
                    method: 'PUT',
                    body: JSON.stringify({ items: this.items })
                });
            } else {
                await apiRequest(`/lists/work_todos?user_id=${this.userId}`, {
                    method: 'POST',
                    body: JSON.stringify({
            ...
            console.error('Failed to save work todos:', error);
        }
    }
    ...
    async archiveOldCompletedItems() {
        ...
        const itemsToKeep = this.items.filter(item => {
            ...
            console.log(`📦 Archiving completed work item: ${item.text}`);
            item.archivedAt = now;
        ...
        if (itemsToArchive.length > 0) {
            ...
            console.log(`📦 Archived ${itemsToArchive.length} completed work items`);
        }
    }
