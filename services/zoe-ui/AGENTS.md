...
- The estate **music card** has three contracts that are easy to break and cheap to keep:
  ...
  - **Layout here is measured, not eyeballed.** At `CF_K=1` the covers fan from y39 (centre) out to y78/y91/y103 and end at y412, so the top strip cannot host a 48px control without clipping artwork — the volume pill lives on the scrub's left flank (x44, clear of `.mscrub` at x360). The transport carries an inert `.tsp` spacer so the 6th button (don't-stop) does not shove play/pause off-centre.
  - Verify with `node dist/test_touch_music_polish.js` (headless 1280×720; asserts fixtures against the live `/api/music/now-playing` + `/api/music/queue/{id}`, including that now-playing still has no `uri`, and writes screenshots). Local gate — no JS lane in CI. **Look at the PNG**: the first volume placement passed every assertion while sitting on top of the cover art.
