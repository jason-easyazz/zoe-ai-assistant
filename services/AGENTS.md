...
- **`zoe-core` KV-prefix contract — nothing turn-varying may sit at the FRONT of a request.** llama.cpp reuses cached KV only for an EXACT common prefix (Gemma's shared-KV + SWA attention has no `--cache-reuse`), so the two front-of-request surfaces are frozen by design: the SYSTEM PROMPT (`extensions/soul.ts` + `extensions/memory.ts`) may only carry text whose inputs are per-PROCESS constants, and the TOOL BLOCK (`extensions/abilities.ts`) is disclosed MONOTONELY over a bounded window (`ZOE_CORE_DISCLOSURE_WINDOW_TURNS`, default 6 turns = zoe-data's replayed `history[-12:]`) rather than recomputed from the last message. Volatile per-turn context goes in the TAIL: the memory packet is returned as a Pi `custom` message, which Pi appends after the user message. `extensions/abilities.ts` must keep calling `setActiveTools` on EVERY turn even when the set is unchanged — that call is the unconditional strip of Pi's coding builtins.
- **The `/api/memories/for-prompt` packet is fetched in ONE place for the core lane: `zoe-core/extensions/memory.ts`.** `zoe_core_client._compose_message` folds in caller-supplied context only; `routers/chat.py` passes `db_memory_context=None` by default (`ZOE_CHAT_INJECT_DB_MEMORY`, off) precisely so the facts are not injected twice. Do not add a second fetch on the python seam.
