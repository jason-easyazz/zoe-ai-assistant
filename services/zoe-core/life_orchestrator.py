...
class LifeOrchestrator:
    ...
    async def _api_get(self, endpoint: str, user_id: str) -> Optional[Dict[str, Any]]:
        """Helper to make GET requests to internal APIs with a timeout."""
        try:
    ...
    async def analyze_everything(self, user_id: str, context: Dict) -> Dict[str, Any]:
        ...
        cache_key = f"life_analysis_{user_id}"
        async with self._cache_lock:
            cached_data = self._cache.get(cache_key)
            if cached_data:
                timestamp, data = cached_data
                if datetime.now() - timestamp < self._cache_ttl:
                    logger.info("Returning cached life analysis.")
                    return data
        ...
        async with self._cache_lock:
            self._cache[cache_key] = (datetime.now(), life_analysis)
