...
async def get_pending_notifications(user_id: str = Query("default")):
    ...
    try:
        ...
        cursor.execute("""
            SELECT n.*, r.title, r.category, r.priority, r.requires_acknowledgment
            FROM notifications n
            JOIN reminders r ON n.reminder_id = r.id
            WHERE n.user_id = ? AND n.is_delivered = FALSE 
            AND n.notification_time <= ?
            ORDER BY n.priority DESC, n.notification_time ASC
