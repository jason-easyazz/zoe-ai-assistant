...
def execute_command(cmd: str, timeout: int = 30, cwd: str = None) -> dict:
    ...
    try:
        ...
        return {
            "success": False,
            "stdout": "",
            "stderr": "Command execution disabled by security policy",
            "code": -1
        }
...
async def clear_redis_cache(session = Depends(require_permission("admin"))):
    ...
    try:
        import subprocess
        # Command execution disabled by security policy
        # result = subprocess.run(["docker", "exec", "zoe-redis", "redis-cli", "FLUSHALL"], capture_output=True, text=True, timeout=10)
        return {"status": "error", "message": "Command execution disabled by security policy"}
    except Exception as e:
...
@router.get("/health")
async def get_all_health(session = Depends(validate_session)):
    catalog = _service_catalog()
    results = {}
