...
async def get_journal_entries(limit: int = 20, user_id: str = "default"):
    ...
    try:
        async with aiosqlite.connect(CONFIG["database_path"]) as db:
            cursor = await db.execute("""
                SELECT id, title, content, mood, mood_score, mood_confidence, 
                       photo, health_info, created_at, updated_at
            ...
            for entry in entries:
                health_info = None
                if entry[7]:  # health_info column
                    try:
                        health_info = json.loads(entry[7])
                    except:
                        health_info = None
...
async def create_journal_entry(entry: JournalEntry):
    ...
    try:
        async with aiosqlite.connect(CONFIG["database_path"]) as db:
            health_info_json = None
            if entry.health_info:
                health_info_json = json.dumps(entry.health_info)
            
            cursor = await db.execute("""
                INSERT INTO journal_entries 
                (user_id, title, content, mood, mood_score, mood_confidence, photo, health_info)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                entry.user_id,
                entry.title,
    ...
    try:
        async with aiosqlite.connect(CONFIG["database_path"]) as db:
            ...
            cursor = await db.execute("""
                ...
                SET title = ?, content = ?, mood = ?, mood_score = ?, 
                    mood_confidence = ?, photo = ?, health_info = ?,
...
async def delete_journal_entry(entry_id: int, user_id: str = "default"):
