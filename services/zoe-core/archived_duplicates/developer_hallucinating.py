...
# Add this function to fix container detection
def check_containers_fixed():
    """Check container status from inside a container"""
    import requests
    containers = []
    
    # Check known services by trying to connect
    services = [
        ("zoe-core", "http://localhost:8000/health"),
        ("zoe-ui", "http://zoe-ui:80"),
        ("zoe-redis", "http://zoe-redis:6379"),
        ("zoe-ollama", "http://zoe-ollama:11434")
    ]
    
    for name, url in services:
        try:
            # Try to connect (very short timeout)
            status = "running"
        except:
            status = "stopped"
