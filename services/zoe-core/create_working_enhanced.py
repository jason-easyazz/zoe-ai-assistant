# Create working enhanced detection
import re
...
# Simple but effective enhanced chat
working_enhanced_chat = '''@app.post("/api/chat/enhanced")
async def enhanced_chat_working(request: dict):
    """Enhanced chat - working name detection"""
    try:
        ...
        pattern1 = re.search(r"my (sister|brother|mum|mom|dad|father|friend)\s+([A-Z][a-z]+)", message, re.IGNORECASE)
        if pattern1:
        ...
        elif re.search(r"([A-Z][a-z]+)\s+is\s+my\s+(sister|brother|mum|dad|friend)", message, re.IGNORECASE):
            pattern2 = re.search(r"([A-Z][a-z]+)\s+is\s+my\s+(sister|brother|mum|dad|friend)", message, re.IGNORECASE)
        ...
        # Pattern 3: Possessive "[Name]'s"
        elif re.search(r"([A-Z][a-z]+)'s", message):
            pattern3 = re.search(r"([A-Z][a-z]+)'s", message)
            person_name = pattern3.group(1).capitalize()
            person = person_name
            relationship = "friend"  # Default
            print(f"Pattern 3 matched: {person_name}")
        
        # Pattern 4: Generic "my [relationship]" (fallback)
        elif re.search(r"my (sister|brother|mum|mom|dad|father)", message, re.IGNORECASE):
            pattern4 = re.search(r"my (sister|brother|mum|mom|dad|father)", message, re.IGNORECASE)
                ...
                async with aiosqlite.connect(db_path) as db:
                    ...
                    if existing:
                        ...
                        if relationship and not person_name and relationship in ["sister", "brother"]:
                            cursor = await db.execute(
                                "SELECT name FROM people WHERE relationship LIKE ? AND user_id = ?", 
                                (f"%{relationship}%", "default")
                            )
                            existing_relations = await cursor.fetchall()
                            if existing_relations:
                                needs_clarification = True
                                existing_names = [r[0] for r in existing_relations]
                                clarification_message = f"I already know about: {', '.join(existing_names)}. What's your {relationship}'s name?"
...
# Remove the old enhanced chat and add the new one
lines = content.split('\n')
new_lines = []
...
for line in lines:
    if '@app.post("/api/chat/enhanced")' in line:
        ...
        new_lines.append(working_enhanced_chat)
        continue
...
print("✅ Working enhanced detection created")
