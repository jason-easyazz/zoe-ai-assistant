...
async def _execute_calendar_create_direct(intent: Intent, user_id: str) -> Optional[str]:
    ...
    if not start_date:
        # No day given (or unparseable) → default to TODAY. A natural quick-add
        # like "add lunch with Jess at 12pm" means today; asking "which day?" every
        # time is worse UX than defaulting. The confirmation names the day ("...
        # today at 12 PM"), so the user can move it if they meant otherwise.
