...
_HERMES_WORKER_OLLAMA_NUM_CTX = 65536
...
def _build_zoe_self_md(
    ...
    lines += [
        ...
        "- llama-server (host) :11434 — local LLM (Gemma 4 E2B)",
        "",
        "## A2A Federation",
...
def _write_hermes_worker_profile_configs() -> dict[str, str]:
    ...
    for config_path in _HERMES_WORKER_CONFIGS:
        try:
            ...
            model_config["ollama_num_ctx"] = _HERMES_WORKER_OLLAMA_NUM_CTX
            model_config["base_url"] = _HERMES_WORKER_BASE_URL
...
def _build_zoe_why() -> str:
    ...
    content = (
        ...
        "task, read the charter section relevant to your change and the graphify map\n"
        "(graphify-out/GRAPH_REPORT.md or the graphify_search tool); honor the\n"
...
def _build_federation_skills_md() -> str:
    """Build FEDERATION_SKILLS.md snapshot from live skill discovery."""
    import datetime as _dt
    lines = [
        "# FEDERATION_SKILLS.md — Live Skill Snapshot",
        f"_Auto-generated by agent_sync at {_dt.datetime.now().strftime('%Y-%m-%d %H:%M')} UTC_",
        "",
        "This file lists the skills discovered from each agent tier. Rebuilt on every agent sync.",
        "",
    ]
    try:
        from skill_discovery import parse_openclaw_skills  # type: ignore[import]
        oc_skills = parse_openclaw_skills()
        lines += [
            f"## OpenClaw Skills ({len(oc_skills)} found)",
            "",
        ]
        for s in oc_skills:
        ...
        lines += [f"## OpenClaw Skills", f"_Error: {exc}_", ""]

    try:
        from skill_discovery import parse_hermes_skills  # type: ignore[import]
        h_skills = parse_hermes_skills()
        lines += [
            f"## Hermes Skills ({len(h_skills)} found)",
            "",
        ]
        for s in h_skills:
            desc = (s.get("description") or "").replace("\n", " ")[:120]
        ...
        lines += [f"## Hermes Skills", f"_Error: {exc}_", ""]
...
def _trigger_graphify_rebuild() -> str:
    ...
    if not _env_enabled("ZOE_AGENT_SYNC_REBUILD_GRAPH"):
        return "skipped (set ZOE_AGENT_SYNC_REBUILD_GRAPH=true to rebuild tracked graphify outputs)"
...
async def run_agent_sync() -> dict:
    ...
    # Write FEDERATION_SKILLS.md
    try:
        fed_skills_md = _build_federation_skills_md()
        _FEDERATION_SKILLS_MD.parent.mkdir(parents=True, exist_ok=True)
        _FEDERATION_SKILLS_MD.write_text(fed_skills_md)
        results["federation_skills_md"] = "updated"
        logger.info("agent_sync: wrote FEDERATION_SKILLS.md (%d chars)", len(fed_skills_md))
    except Exception as exc:
