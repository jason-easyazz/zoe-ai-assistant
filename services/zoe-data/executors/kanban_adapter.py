...
def _harness_implement_hint(issue: dict | None = None) -> str:
    ...
    if source == "engineering_blocker_followup":
        ...
        elif blocker in {"IMPLEMENT_BUDGET", "IMPLEMENT_HANDOFF_DRIFT", "ITERATION_BUDGET"}:
            focused_test = (
                "services/zoe-data/tests/test_main_multica_poll.py::"
                "test_record_blocked_multica_chain_creates_iteration_budget_followup"
            )
    ...
    def _build_body(self, phase: str, issue: dict, identifier: str, *, mode: str | None = None) -> str:
        ...
        if phase in {"implement", "implement_revision"}:
            ...
            return common + overnight_hint + (
                ...
                " `pwd && git branch --show-current` before editing, testing, committing, pushing,"
                " or opening a PR. The working directory must match the task `workspace_path` shown"
                " by Kanban. If it shows the live repo checkout instead of the task worktree, call"
                " `kanban_block` with BLOCKER=WORKTREE_PATH_VIOLATION immediately; do not `cd` into"
                f" `{zoe_repo_root()}` for git, test, patch, or PR commands.\n"
