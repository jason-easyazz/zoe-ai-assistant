    ...
    def _build_body(self, phase: str, issue: dict, identifier: str, *, mode: str | None = None) -> str:
        ...
        if phase == "implement":
            ...
            return common + overnight_hint + (
                ...
                " Start with `kanban_show`, then run `gh pr view <PR_URL> --json url,number,headRefName,"
                ...
                " Check out the existing PR branch with `gh pr checkout <number>`, address only the"
                " unresolved review/Greptile/CI action list, run focused tests plus validators, commit,"
                " `git push`, and report the SAME PR_URL. Run Python tests with"
                ...
                " to change. If checkout fails or the action list is"
                " ambiguous, call `kanban_block` with BLOCKER=PR_REVISION_BLOCKED.\n"
