...
- Background loops must not fork on the event loop thread. `asyncio.create_subprocess_exec` forks synchronously on the loop; in this large multi-threaded process a fork can deadlock pre-exec and freeze the whole API (live outage 2026-06-29: one wedged `hermes kanban` spawn made every endpoint, `/health` included, time out for days). Run-to-completion CLI spawns (`executors/kanban_adapter.py`'s `_spawn_cli`, `routers/voice_tts.py`'s `_spawn_tts_cli` for the espeak-ng/ffmpeg TTS fallbacks) go through `subprocess.run` in a small dedicated thread pool + coroutine-side `wait_for` — a `wait_for` around the coroutine alone cannot fire when the loop thread itself is blocked. The one long-lived spawn (`zoe_core_client.py`'s Pi RPC brain subprocess, held open for the worker's life) uses the same off-loop `subprocess.Popen` fork+exec via `_spawn_pi_process`, then wraps the already-open pipe fds in asyncio stream transports (`connect_read_pipe`/`connect_write_pipe`, which don't fork) so the rest of the worker keeps its async stdin/stdout interface.
