...
- cancel_reminder DELETEd the row BEFORE cancel_job() — a cancel failure left an
  orphan job that fires against a row that no longer exists.

Fixed behaviour:
- schedule_reminder compensates by deleting the orphan row if register_job fails.
- cancel_reminder cancels the scheduler job FIRST, then deletes the row.
"""
import contextlib
...
async def test_cancel_absent_job_still_removes_stale_row(monkeypatch):
    ...
    monkeypatch.setattr(reminders, "cancel_job", lambda job_id: (ops.append(("cancel", job_id)) or False))
