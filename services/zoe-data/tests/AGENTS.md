...
- **`~/.zoe-voice-samples` is an UNCURATED live corpus — never assert on one arbitrary member.** It is a save-everything capture that grows on the box: measured 2026-08-04 over 1151 files, ~11% of the usable recordings are not speech (false wakes, near-silence, background TV), 4 are 24kHz rather than 16k mono, and one is not even a valid RIFF file. Filenames are `HHMMSS_millis`, so they sort by **time of day, not date** — a recording made later but earlier in the day silently becomes `sorted(...)[0]`. `test_voice_barge_in.py` did exactly that and went red for weeks with the model and code untouched. Host-only tests over this corpus must sample a deterministic slice, SKIP off-format/unreadable members rather than fail on them, and assert a corpus-level property (a majority clearing the threshold) instead of a single file's score.
