...
- Agent-security guards: `test_agent_safety.py` (shell-injection + SSRF policy) imports only the stdlib-only `agent_safety` module, so it is CI-safe and wired into the slim list in `validate.yml`. The companion `test_agent_safety_wiring.py` (behavioral `_bash` + router/SSRF call-site checks) imports `zoe_agent`/`routers.system` and so stays Jetson-only (it `importorskip`s those modules).
