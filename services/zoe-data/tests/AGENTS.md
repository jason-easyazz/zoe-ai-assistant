...
- **A test that wants REAL audio must run in CI too — parametrize the source, never gate the whole test on the corpus.** The replay corpus (`~/.zoe-voice-samples`) exists only on the Jetson, so a suite bound to it is a suite CI does not run. The convention is a `params=["synthetic", "corpus"]` fixture: the synthetic arm is deterministic and ALWAYS runs, the corpus arm skips when the box has no usable recording, and **both arms assert exactly the same thing** — that is what makes the CI arm worth having. `test_livekit_audio_frame_bytes.py` and `test_livekit_vad_segmentation.py` are the reference. Combine with the uncurated-corpus rule below: the corpus arm still has to stride, skip off-format members, and reject near-silent false wakes.
