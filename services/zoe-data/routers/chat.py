...
async def run_openclaw_agent(message: str, session_id: str, user_id: str = "family-admin") -> str:
    """Route through OpenClaw for full memory, personality, and tool access."""
    return await openclaw_cli(message, session_id, user_id)
...
async def _stream_openclaw_assistant_ag(
    ...
    async for line in iter_text_message_chunks(enc, recorder, assistant_message_id, clean_text):
        yield line
    yield recorder.emit(
        enc,
...
async def chat_stream_generator(message: str, session_id: str, user_id: str, *, force_openclaw: bool = False):
    ...
    try:
        ...
        if intent:
            ...
            if result:
                ...
                yield emit(
                    StateSnapshotEvent(
                        ...
                        snapshot={"status": "generating", "model": "Zoe (LLM fallback)"},
                    )
                )
                response_text = await run_openclaw_agent(message_for_processing, session_id, user_id)
            ...
            yield emit(
                StateSnapshotEvent(
                    type=EventType.STATE_SNAPSHOT,
                    snapshot={"status": "generating", "model": "Zoe"},
                )
            )
            response_text = await run_openclaw_agent(message_for_processing, session_id, user_id)
            _, actions = _extract_ui_actions(response_text)
...
async def chat(request: Request, user: dict = Depends(get_current_user), stream: bool = True):
    ...
    if stream:
        return StreamingResponse(
            chat_stream_generator(message, session_id, user_id, force_openclaw=force_openclaw),
...
async def whatsapp_connect(request: Request, user: dict = Depends(get_current_user)):
    ...
    response_text = await run_openclaw_agent(guidance, session_id, user_id)
