...
- **Callers send `Content-Type: application/json`; `strict_content_type` stays TRUE.** FastAPI rejects (422) a non-empty body on a non-form route unless the header is `application/json` or an `application/…+json` suffix. Do NOT pass `strict_content_type=False` to `FastAPI(...)` to accommodate a caller — fix the caller. Server-side Python must use httpx/aiohttp `json=` (never `data=json.dumps(...)` without the header); browser JS must set the header explicitly or go through a wrapper that does. OIDC routes are `Form(...)`-based and exempt. Pinned by `tests/test_strict_content_type.py`.
- **Passcode `pattern=` constraints must be anchored `^…$`.** pydantic ≥2.9 applies `pattern` with `re.search`, so an unanchored pattern silently accepts any string that merely CONTAINS a match — a real loosening of a login credential. Pinned by `tests/test_passcode_pattern_anchoring.py`; add new passcode fields to its model list.
