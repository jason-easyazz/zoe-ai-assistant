...
- **A module deploys from its own `docker-compose.module.yml` under its own compose project** (`docker compose -p <name> -f modules/<name>/docker-compose.module.yml up -d`), as `omnigent` does. `tools/generate_module_compose.py` writes `docker-compose.generated-modules.yml`, but nothing consumes that file today and the generator cannot represent `omnigent` (it hardcodes `zoe-network` as the only top-level network) — forcing it through fails loudly at the generator's own `docker compose config` step, not silently. **Keep `omnigent` out of `config/modules.yaml`.** Pinned by `tests/unit/modules/test_generate_module_compose.py` (`test_generator_cannot_represent_omnigent`, `test_omnigent_is_not_enabled_in_modules_yaml`); if either goes red, re-check this bullet before "fixing" the test. Do not hand-edit generated compose output; the generator rejects non-slug module names (no path traversal).
...
- **That rule is machine-checked, STRUCTURALLY.** `tools/validate_module.py` resolves the route gate through the AST and the publish addresses through the parsed compose — never by text search, because a comment satisfies a grep for `/tools/` or `127.0.0.1` and a validator that can be satisfied without the property is not a validator. The normative reference is the contract section of [docs/guides/MODULE_SYSTEM.md](../docs/guides/MODULE_SYSTEM.md); `tests/unit/modules/test_validate_module_security.py` pins both directions (the documented form passes, each violation fails). `omnigent` is not exempted — it is container-only, so the route checks have nothing to resolve, and its one published port is already loopback-bound.
- **`omnigent`'s MCP config (`omnigent/.mcp.json`, bind-mounted over `/workspace/.mcp.json`) must never spawn a stdio Serena again.** Each container-spawned server was ~900 MB RSS on a 15.6 GB box shared with llama-server + Kokoro. It uses the host's shared `serena-mcp.service` over `zoe-codeintel` — an `internal` network with exactly one member, pinned at `172.28.0.2` — fronted by `scripts/setup/systemd/system/serena-bridge.{socket,service}`. The bridge's `IPAddressAllow=`, not the network, is the access control (a bridge GATEWAY is reachable from every container on the host; only container-to-container across networks is blocked). Keep the pinned address, the subnet, the MCP url and the socket unit in agreement — `tests/unit/modules/test_omnigent_mcp_config.py` fails if they drift. Adding a second member to `zoe-codeintel` widens whole-repo read access and is out of scope for any module change. **The same rule covers Codex in the container:** `/root/.codex/config.toml` lives in the `omnigent-codex` volume, so its MCP tables are owned by the tracked template `omnigent/codex-mcp.toml` (baked into the image, rewritten on every boot by `entrypoint.sh`, everything else in the file kept). Change the serena URL or the codebase-memory launcher in `.mcp.json` and `codex-mcp.toml` together; never hand-patch the file inside the volume as the fix — that is exactly what silently reverted (B0.11). `tests/unit/test_omnigent_entrypoint_codex_mcp.py` pins the seed's behaviour. Provenance of every in-container config surface: [docs/knowledge/omnigent-container-config.md](../docs/knowledge/omnigent-container-config.md).
