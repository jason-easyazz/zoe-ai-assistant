...
class MediaController:
    ...
    async def play(
        ...
        # Add track to front of queue (position 0)
        ...
        # Pre-fetch next track in queue for faster skip
        import asyncio
        asyncio.create_task(self._prefetch_next_track(user_id, target_device_id))
    ...
    async def skip(self, user_id: str, device_id: Optional[str] = None) -> Dict:
        ...
        logger.info(f"skip() called for user={user_id}, device={device_id}")
        ...
        logger.info(f"skip() next_track: {next_track}")
        ...
        if next_track:
            ...
            logger.info(f"skip() queue empty for user={user_id}")
    ...
    async def remove_from_queue(
        ...
        Args:
            position: If specified, removes the track at this specific position.
                     This allows removing duplicates individually.
        """
        try:
            conn = sqlite3.connect(DB_PATH)
            cursor = conn.cursor()
            ...
            if position is not None:
                # Remove by position (handles duplicates correctly)
                cursor.execute("""
                    SELECT id FROM music_queue
                    WHERE user_id = ? AND position = ? AND (device_id = ? OR device_id IS NULL)
                """, (user_id, position, device_id))
                row = cursor.fetchone()
                
                if not row:
                    conn.close()
                    return {"success": False, "error": "Track not found at position"}
                
                removed_position = position
                
                # Delete by row ID
                cursor.execute("DELETE FROM music_queue WHERE id = ?", (row[0],))
            else:
                # Legacy: remove first instance by track_id
                cursor.execute("""
                    SELECT id, position FROM music_queue
    ...
    async def _prefetch_next_track(self, user_id: str, device_id: Optional[str]) -> None:
        """Pre-fetch stream URL for next track in queue (runs in background)."""
        try:
            ...
            cursor.execute("""
                SELECT track_id FROM music_queue
                WHERE user_id = ? AND (device_id = ? OR device_id IS NULL)
                ORDER BY position LIMIT 1
            """, (user_id, device_id))
            ...
            if row and row["track_id"]:
                # Pre-fetch stream URL (this caches it for 5 hours)
                logger.debug(f"Pre-fetching stream URL for next track: {row['track_id']}")
                await self.youtube.get_stream_url(row["track_id"])
                
        except Exception as e:
            logger.debug(f"Pre-fetch failed (non-critical): {e}")
    ...
    async def _pop_queue(self, user_id: str, device_id: Optional[str]) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(f"_pop_queue for user={user_id}, device={device_id}: found={'yes' if row else 'no'}")
            
            if row:
                track_id = row["track_id"]
                track_title = row.get("track_title", "Unknown")
                logger.info(f"_pop_queue: popping track_id={track_id}, title={track_title}")
                # Delete it
