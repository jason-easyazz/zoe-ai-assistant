...
- CI on GitHub runners must not **install** `-r requirements.txt` (Jetson-only packages pull ~3 GB of PyTorch); use a slim explicit install list. **RESOLVING a manifest is a different thing and IS allowed** — `validate.yml`'s `deps-resolvable` job runs `pip install --dry-run --report` over every requirements file, which reads metadata only and builds no environment. That job is what proves the manifests are still satisfiable and that the `numpy<2` / `chromadb` / `bcrypt` ceilings still hold; the slim list proves nothing about them because it never reads the manifests. Keep the pinned entries in the slim list in lockstep with `services/zoe-data/requirements.txt` — anything left unpinned there resolves to the newest release at CI time and silently tests a different version than production runs.
...
- **Living under `tests/` is NOT evidence that CI runs it — check collection, not presence.** `pytest.ini` pins `python_classes = Test*`, so a file whose suite is a differently-named class (`class ZoeCapabilityTest:` + `if __name__ == "__main__"`) is imported and then collects **nothing**. Measured 2026-08-06: **9 of 20** `tests/integration/test_*.py` files collect **0** tests (`test_all_capabilities`, `test_all_systems`, `test_hallucination_benchmark`, `test_memories`, `test_p0_validation`, `test_people_full_integration`, `test_people_system`, `test_session_management`, `test_suite`), and collecting the directory raises **7 collection errors**. These are the ad-hoc scripts the bullet above describes, sitting in a directory `self-hosted-tests.yml` runs. Before citing an integration test as coverage, run `pytest <file> --collect-only -q` and read the count.
