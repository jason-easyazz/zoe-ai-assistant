#!/usr/bin/env python3
"""
Add Dog Treats to the admin user's shopping list
"""

import asyncio
import httpx

async def add_to_admin_user():
    """Add Dog Treats to admin user via chat API"""
    
    print("="*80)
    print("🐕 Adding Dog Treats to ADMIN user's shopping list")
    print("="*80)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if list_response.status_code == 200:
                    ...
                    print(f"\n🛒 Shopping List for admin ({len(items)} items):")
                    for item in items:
                        status = "✓" if item.get("completed") else "○"
                        print(f"   {status} {item.get('text')}")
                    
                    if any("dog treats" in item.get("text", "").lower() for item in items):
                        print(f"\n✅ SUCCESS: Dog Treats is now in admin's shopping list!")
                        print(f"\n🔄 Now refresh lists.html in your browser to see it!")
                        return True
                    else:
