...
class SystemTester:
    ...
    async def get_jwt_token(self) -> Optional[str]:
        """
        Get JWT token from zoe-auth service for authenticated requests
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    if data.get("success") and data.get("session_id"):
                        self.jwt_token = data["session_id"]
                        logger.info(f"✅ JWT token obtained successfully")
                        return self.jwt_token
                    else:
                        logger.warning(f"⚠️ Login succeeded but no session_id: {data}")
                else:
                    logger.warning(f"⚠️ Auth login failed: {response.status_code} - {response.text[:200]}")
                
                # If login failed, try guest login as fallback
                response = await client.post(
                    f"{AUTH_API_URL}/api/auth/guest",
                    json={"device_info": {"device_type": "test"}}
                )
                ...
                if response.status_code == 200:
                    ...
                    if data.get("success") and data.get("session_id"):
                        self.jwt_token = data["session_id"]
                        logger.info(f"✅ Guest JWT token obtained as fallback")
                        return self.jwt_token
                
                logger.warning("⚠️ Could not obtain JWT token (user login and guest fallback failed)")
                return None
    ...
    async def test_chat_api(self, prompt: str, query_type: str = "conversation") -> dict:
        """Test full chat API endpoint with authentication"""
        try:
