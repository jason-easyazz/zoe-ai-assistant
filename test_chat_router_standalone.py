#!/usr/bin/env python3
"""
Standalone Chat Router Test
===========================

Test the reliable chat router without requiring the full system to be running.
"""
...
# Add the services path to import the chat router
sys.path.append('/workspace/services/zoe-core')

def test_chat_router_standalone():
    """Test the chat router directly without HTTP requests"""
    print("🧪 TESTING CHAT ROUTER STANDALONE")
    print("=" * 50)
    ...
    try:
        ...
        for i, scenario in enumerate(test_scenarios, 1):
            ...
            try:
                ...
                # Test the reliable chat function directly
                response = reliable_chat(chat_msg, "test_user")
        ...
        if overall_score >= 95:
            ...
            print(f"🚀 The chat router is working perfectly!")
        else:
            print(f"\n⚠️ Chat router needs improvement to reach 100%")
            print(f"🎯 Target: 95%+ overall score")
        
        # Save results
        with open("chat_router_standalone_test_results.json", "w") as f:
            json.dump(results, f, indent=2)
        
        print(f"\n💾 Results saved to: chat_router_standalone_test_results.json")
