...
cat > "$PROJECT_DIR/services/zoe-core/endpoints_addition.py" << 'ENDPOINTS_EOF'
# =============================================================================
...
if [ -f "$MAIN_PY" ]; then
    ...
    cat > "$PROJECT_DIR/services/zoe-core/main_enhanced.py" << 'COMPLETE_EOF'
"""
...
async def chat_endpoint(chat_msg: ChatMessage):
    ...
    try:
        ...
        response = f"Hello! You said: {chat_msg.message}. I'm Zoe and the enhanced backend is working! 🚀"
        
        # Save message to database
        async with aiosqlite.connect(CONFIG["database_path"]) as db:
            await db.execute("""
                INSERT INTO messages (role, content, user_id)
                VALUES (?, ?, ?)
            """, ("user", chat_msg.message, chat_msg.user_id))
            
            await db.execute("""
                INSERT INTO messages (role, content, user_id)
                VALUES (?, ?, ?)
            """, ("assistant", response, chat_msg.user_id))
...
cat > "$PROJECT_DIR/deploy_enhanced_backend.sh" << 'DEPLOY_EOF'
#!/bin/bash
